...
def complete_storage_page_plan(
    keys: list[str], host_indices: Any, *, page_size: int
) -> tuple[list[str], list[tuple[int, int]]]:
    """Return only whole physical pages eligible for lower-tier persistence.

    A radix node may end in the middle of an SGLang page.  Its hash chain can
    still contain a final key for that partial token suffix, but Central I/O
    cannot safely make the physical page durable or reclaimable: it may also
    contain another radix node's KV.  This helper retains only contiguous,
    page-aligned complete groups, with one key per returned page.  It performs
    Python conversion once per page, never once per token slot.
    """
    if page_size <= 0:
        raise ValueError("storage page size must be positive")
    flat = host_indices.detach().reshape(-1)
    complete_group_count = int(flat.numel()) // page_size
    if complete_group_count == 0 or not keys:
        return [], []
    import torch

    groups = flat[: complete_group_count * page_size].reshape(
        complete_group_count, page_size
    )
    starts = groups[:, 0]
    expected = starts.unsqueeze(1) + torch.arange(
        page_size, device=groups.device, dtype=groups.dtype
    )
    valid = (starts.remainder(page_size) == 0) & torch.all(groups == expected, dim=1)
    valid_positions = torch.nonzero(valid, as_tuple=False).flatten().cpu().tolist()
    ...
    for position in valid_positions:
        ...
        page_ids.append(int(starts[position].item()) // page_size)

    ranges: list[tuple[int, int]] = []
    for page_id in page_ids:
        if ranges and page_id == ranges[-1][0] + ranges[-1][1]:
            start, count = ranges[-1]
            ranges[-1] = (start, count + 1)
        else:
            ranges.append((page_id, 1))
    return selected_keys, ranges
...
class CentralIOClient:
    ...
    def storage_write_pages(
        self, keys: list[str], ranges: list[tuple[int, int]]
    ) -> dict[str, Any]:
        """Ask the agent to persist this model's live host-KV pages."""
        return self._call(
            {
                "op": "storage_write_pages",
                "model_id": self.model_id,
                "keys": keys,
                "page_ranges": [[start, count] for start, count in ranges],
            }
        )

    def storage_read_pages(
        self, keys: list[str], ranges: list[tuple[int, int]]
    ) -> dict[str, Any]:
        """Ask the agent to restore durable pages into this model's lease."""
        return self._call(
            {
                "op": "storage_read_pages",
                "model_id": self.model_id,
                "keys": keys,
                "page_ranges": [[start, count] for start, count in ranges],
            }
    ...
    def storage_existing_prefix(self, keys: list[str]) -> int:
        """Return how many leading page keys are already durable for this model."""
        return int(
            self._call(
                {
                    "op": "storage_existing_prefix",
                    "model_id": self.model_id,
                    "keys": keys,
                }
            )["page_count"]
    ...
    def storage_status(self, operation_id: int) -> dict[str, Any]:
        return self._call(
            {
                "op": "storage_status",
                "model_id": self.model_id,
...
class _ModelState:
    ...
    # Target capacity is only scheduler intent.  These fields record the
    # separate moment at which a donor/recipient handshake actually changes
    # this model's usable lease.
    last_effective_capacity_change_ns: int | None = None
    last_effective_capacity_change_reason: str | None = None
...
class _StorageOperation:
    """One agent-owned lower-tier transfer over complete host KV pages."""
...
class CentralIOAgent:
    ...
    def __init__(
        ...
        storage_root = os.getenv("SGLANG_CENTRAL_IO_STORAGE_PATH")
        if storage_root:
            self._init_storage_proxy(storage_root)
    ...
    @staticmethod
    def _storage_worker_count() -> int:
        workers = int(os.getenv("SGLANG_CENTRAL_IO_STORAGE_WORKERS", "1"))
        if workers <= 0:
            raise ValueError("SGLANG_CENTRAL_IO_STORAGE_WORKERS must be positive")
        return workers

    def _init_storage_proxy(self, root: str) -> None:
        """Enable raw page storage owned by the Central I/O agent.

        The regular SGLang storage backends receive host Tensor views in the
        model process.  That is incompatible with Central I/O because the
        actual pinned bytes and their physical mapping exist only here.  This
        proxy deliberately operates on complete pages from the agent mmap,
        leaving host allocation and quota ownership unchanged until a durable
        acknowledgement lets HiRadix release the host copy.
        """
        storage_root = Path(root)
        storage_root.mkdir(parents=True, exist_ok=True)
        self._storage_root = storage_root
        self._storage_operations: dict[int, _StorageOperation] = {}
        self._next_storage_operation_id = 0
        self._storage_executor = ThreadPoolExecutor(
            max_workers=self._storage_worker_count(),
            thread_name_prefix="central-io-storage",
        )

    def _close_storage_proxy(self) -> None:
        executor = getattr(self, "_storage_executor", None)
        if executor is not None:
            executor.shutdown(wait=True)
            self._storage_executor = None

    def _storage_page_descriptors(
        self, state: _ModelState, page_ranges: list[tuple[int, int]]
    ) -> list[tuple[int, int]]:
        """Translate model-local live pages to raw central-pool byte slices."""
        page_limit = state.max_capacity // state.page_size
        ranges = self._validate_page_ranges(page_ranges, page_limit)
        if not all(
            state.reserved_pages.contains(page_start, page_count)
            for page_start, page_count in ranges
        ):
            raise ValueError("Central I/O storage requires pages live in the model lease")
        page_bytes = self._page_bytes(state)
        descriptors: list[tuple[int, int]] = []
        for page_start, page_count in ranges:
            for page_id in range(page_start, page_start + page_count):
                segment, local_page = self._segment_for_page(state, page_id)
                descriptors.append(
                    (segment.byte_offset + local_page * page_bytes, page_bytes)
                )
        return descriptors

    @staticmethod
    def _storage_key_path(root: Path, state: _ModelState, key: str) -> Path:
        """Derive a filesystem-safe, geometry-scoped path for one KV page."""
        if not isinstance(key, str) or not key:
            raise ValueError("Central I/O storage key must be a non-empty string")
        namespace = (
            f"{state.model_id}|{state.layer_count}|{state.head_count}|"
            f"{state.head_dim}|{state.page_size}|{state.dtype}"
        )
        namespace_id = hashlib.blake2b(namespace.encode(), digest_size=16).hexdigest()
        key_id = hashlib.blake2b(key.encode(), digest_size=32).hexdigest()
        return root / namespace_id / key_id

    def _start_storage_operation(
        self, state: _ModelState, direction: str, keys: list[str], page_ranges: list[tuple[int, int]]
    ) -> tuple[_StorageOperation, list[tuple[int, int]], list[Path]]:
        if direction not in {"write", "read"}:
            raise ValueError("Central I/O storage direction must be write or read")
        root = getattr(self, "_storage_root", None)
        if root is None:
            raise RuntimeError("Central I/O storage proxy is not configured")
        descriptors = self._storage_page_descriptors(state, page_ranges)
        if len(keys) != len(descriptors):
            raise ValueError("Central I/O storage requires one key per complete page")
        paths = [self._storage_key_path(root, state, key) for key in keys]
        operation_id = getattr(self, "_next_storage_operation_id", 0)
        self._next_storage_operation_id = operation_id + 1
        operation = _StorageOperation(
            operation_id=operation_id,
            model_id=state.model_id,
            direction=direction,
            page_count=len(descriptors),
            state="writing" if direction == "write" else "reading",
        )
        operations = getattr(self, "_storage_operations", None)
        if operations is None:
            operations = {}
            self._storage_operations = operations
        operations[operation_id] = operation
        return operation, descriptors, paths

    def _storage_write_pages(
        self, state: _ModelState, keys: list[str], page_ranges: list[tuple[int, int]]
    ) -> dict[str, Any]:
        """Asynchronously persist complete live pages and report durable ack."""
        operation, descriptors, paths = self._start_storage_operation(
            state, "write", keys, page_ranges
        )

        def write_pages() -> None:
            try:
                for page_index, ((offset, byte_count), path) in enumerate(
                    zip(descriptors, paths), start=1
                ):
                    path.parent.mkdir(parents=True, exist_ok=True)
                    temporary = path.with_name(
                        f".{path.name}.{operation.operation_id}.tmp"
                    )
                    with open(temporary, "wb") as output:
                        output.write(self._mapping[offset : offset + byte_count])
                        output.flush()
                        os.fsync(output.fileno())
                    os.replace(temporary, path)
                    with self._lock:
                        operation.completed_pages = page_index
                operation.state = "durable"
            except Exception as error:
                operation.error = repr(error)
                operation.state = "failed"
            finally:
                operation.done.set()

        self._storage_executor.submit(write_pages)
        return {"operation_id": operation.operation_id, "state": operation.state}

    def _storage_read_pages(
        self, state: _ModelState, keys: list[str], page_ranges: list[tuple[int, int]]
    ) -> dict[str, Any]:
        """Asynchronously restore durable pages without partial host mutation."""
        operation, descriptors, paths = self._start_storage_operation(
            state, "read", keys, page_ranges
        )

        def read_pages() -> None:
            try:
                payloads: list[bytes] = []
                for (_offset, byte_count), path in zip(descriptors, paths):
                    with open(path, "rb") as source:
                        payload = source.read()
                    if len(payload) != byte_count:
                        raise ValueError("Central I/O storage page size mismatch")
                    payloads.append(payload)
                for page_index, ((offset, _byte_count), payload) in enumerate(
                    zip(descriptors, payloads), start=1
                ):
                    self._mapping[offset : offset + len(payload)] = payload
                    with self._lock:
                        operation.completed_pages = page_index
        ...
        self._storage_executor.submit(read_pages)
        return {"operation_id": operation.operation_id, "state": operation.state}

    def _storage_existing_prefix(self, state: _ModelState, keys: list[str]) -> int:
        """Count durable page keys until the first miss, without touching host KV.

        Prefetch is prefix-ordered.  Returning a contiguous prefix prevents a
        later missing page from exposing a partially restored host prefix to
        HiRadix.
        """
        root = getattr(self, "_storage_root", None)
        if root is None:
        ...
        count = 0
        for key in keys:
            if not self._storage_key_path(root, state, key).is_file():
                break
            count += 1
        return count

    def _storage_operation_status(self, operation_id: int) -> dict[str, Any]:
        try:
            operation = self._storage_operations[operation_id]
        except (AttributeError, KeyError) as error:
            raise ValueError("unknown Central I/O storage operation") from error
        return {
            ...
            "completed_pages": operation.completed_pages,
            "error": operation.error,
        }

    def _wait_storage_operation(
        self, operation_id: int, timeout_s: float | None = None
    ) -> dict[str, Any]:
        try:
            operation = self._storage_operations[operation_id]
        except (AttributeError, KeyError) as error:
            raise ValueError("unknown Central I/O storage operation") from error
        if not operation.done.wait(timeout_s):
            raise TimeoutError("Central I/O storage operation did not finish")
        return self._storage_operation_status(operation_id)
    ...
    def _commit_live_page_reclaim(
        ...
        detach_started = time.perf_counter()
        ...
        detach_ms = (time.perf_counter() - detach_started) * 1000
        ...
        reclaim_pages = sum(count for _, count in page_ranges)
        ...
        self._record_effective_capacity_change(state, "donor_detach")
        ...
        timing_ms = {
            ...
            "agent_detach_pages": detach_ms,
            "agent_submit_scrub": submit_ms,
            "reclaim_pages": reclaim_pages,
        }
        print(
            "[CENTRAL-IO] page-reclaim-start "
            f"model={state.model_id} id={reclaim_id} "
            f"page_ranges={page_ranges} bytes={byte_count} timing_ms={timing_ms}",
            flush=True,
        )
        return {
    ...
    def _reclaim_allocator_free_pages(
        ...
        reclaim = self._commit_live_page_reclaim(state, ranges)
        reclaim.setdefault("timing_ms", {})["agent_select_free_pages"] = select_ms
        return reclaim
    ...
    @staticmethod
    def _record_effective_capacity_change(
        state: _ModelState, reason: str
    ) -> int:
        """Mark an acknowledged allocator lease change for runtime auditing."""
        changed_ns = time.monotonic_ns()
        state.last_effective_capacity_change_ns = changed_ns
        state.last_effective_capacity_change_reason = reason
        return changed_ns
    ...
    def _pending_release_capacity(self, state: _ModelState) -> int:
        """Capacity fenced for donor shrink but not yet allocator-acknowledged."""
        return sum(
            sum(page_count for _, page_count in transfer.page_ranges) * state.page_size
            for transfer in getattr(self, "_pending_shrinks", {}).values()
            if transfer.model_id == state.model_id
        )
    ...
    def _report_local_residency(
        ...
        try:
            ...
            # The epoch deduplicates one reclaim-loss event in newer
            # schedulers.  It is intentionally optional on the wire so an
            # already-running allocator that only knows the older health
            # report cannot make Central I/O reject an otherwise valid
            # admission snapshot during a rolling update.
            report["retention_debt_epoch"] = int(
                request.get("retention_debt_epoch", 0)
            )
        ...
        if action not in {"idle", "prepare", "emergency", "quota_deficit"}:
        ...
        return {
            ...
            "models": {
                state.model_id: {
                    ...
                    "pending_release_capacity": self._pending_release_capacity(state),
                    "quota_target": state.quota_target,
                    "quota_target_set_ns": state.quota_target_set_ns,
                    "last_effective_capacity_change_ns": state.last_effective_capacity_change_ns,
                    "last_effective_capacity_change_reason": state.last_effective_capacity_change_reason,
    ...
    def _ack_prepared_grow(self, state: _ModelState, transfer_id: int) -> dict[str, Any]:
        ...
        effective_change_ns = self._record_effective_capacity_change(
            state, "recipient_allocator_ack"
        )
        ...
        return {
            ...
            "effective_capacity": state.capacity,
            "pending_capacity": 0,
            "effective_capacity_change_ns": effective_change_ns,
    ...
    def _handle(self, request: dict[str, Any]) -> dict[str, Any]:
        ...
        if op == "reserve_pages":
            ...
            if any(
                ...
                raise ValueError("cannot reserve a draining host page")
            if any(
                segment.pending_transfer_id is not None
                for segment, _, _ in locations
            ):
        ...
        if op == "restore":
            ...
            return {}
        if op == "storage_write_pages":
            if not state.dynamic_page_leases:
                raise ValueError("storage proxy requires dynamic page leases")
            with self._lock:
                return self._storage_write_pages(
                    state,
                    list(request["keys"]),
                    [tuple(item) for item in request["page_ranges"]],
                )
        if op == "storage_read_pages":
            if not state.dynamic_page_leases:
                raise ValueError("storage proxy requires dynamic page leases")
            with self._lock:
                return self._storage_read_pages(
                    state,
                    list(request["keys"]),
                    [tuple(item) for item in request["page_ranges"]],
                )
        if op == "storage_existing_prefix":
            if not state.dynamic_page_leases:
                raise ValueError("storage proxy requires dynamic page leases")
            with self._lock:
                return {
                    "page_count": self._storage_existing_prefix(
                        state, list(request["keys"])
                    )
                }
        if op == "storage_status":
            status = self._storage_operation_status(int(request["operation_id"]))
            if status["model_id"] != state.model_id:
                raise ValueError("Central I/O storage operation belongs to another model")
            return status
        if op == "grow_range":
            with self._lock:
    ...
    def warmup_restore_operator(self) -> None:
        """Compile the restore operator before accepting serving clients."""
        import torch

        with self._cuda_lock:
            torch.cuda.set_device(self._registration_device)
            _load_restore_extension()
