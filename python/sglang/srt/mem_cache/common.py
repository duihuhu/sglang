...
def write_cache_indices(
    ...
    if support_triton(get_global_server_args().attention_backend):
        # Central I/O registration can temporarily select another CUDA device
        # while it opens IPC mappings. Triton launches against the current
        # CUDA context, so restore the concrete worker device before launch.
        if req_to_token_pool.req_to_token.device.type == "cuda":
            torch.cuda.set_device(req_to_token_pool.req_to_token.device)
        if os.getenv("SGLANG_DEBUG_REQ_TO_TOKEN_DEVICE") == "1":
            logger.warning(
                "req-to-token devices: current=cuda:%s req_to_token=%s "
                "req_indices=%s prefix_lens=%s seq_lens=%s extend_lens=%s out_cache=%s",
                torch.cuda.current_device(),
                req_to_token_pool.req_to_token.device,
                req_pool_indices_tensor.device,
                prefix_lens_tensor.device,
                seq_lens_tensor.device,
                extend_lens_tensor.device,
                out_cache_loc.device,
            )
...
def alloc_for_extend(
    ...
    # The request-to-token pool is allocated on the model worker's concrete
    # CUDA device. `batch.device` is often the generic string "cuda", which
    # resolves to cuda:0 and is wrong for workers on GPU1+.
    pool_device = batch.req_to_token_pool.req_to_token.device

    # Create tensors for allocation
    prefix_lens_cpu = torch.tensor(batch.prefix_lens, dtype=torch.int64)
    extend_lens_cpu = torch.tensor(batch.extend_lens, dtype=torch.int64)
    prefix_lens_device = prefix_lens_cpu.to(pool_device, non_blocking=True)
    seq_lens_device = batch.seq_lens_cpu.to(pool_device, non_blocking=True)
    extend_lens_device = extend_lens_cpu.to(pool_device, non_blocking=True)
    ...
    req_pool_indices_device = req_pool_indices_cpu.to(pool_device, non_blocking=True)
    ...
    if batch.tree_cache.page_size == 1:
        ...
        last_loc = [
            (t[-1:] if len(t) > 0 else torch.tensor([-1], device=pool_device))
