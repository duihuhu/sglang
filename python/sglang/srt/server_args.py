...
class ServerArgs:
    ...
    def _handle_a2a_moe(self):
        ...
        if self.moe_a2a_backend == "ampere_ep":
            assert (
                resolved_view(self).enable_dp_attention and self.dp_size == self.tp_size
            ), (
                "Ampere EP (FlashInfer MoeAlltoAll) requires dp_size == tp_size and "
                "--enable-dp-attention"
            )
            if self.deepep_mode != "auto":
                logger.warning("--deepep-mode is ignored for ampere_ep MoE A2A")
            allowed_runners = (
                "triton",
                "triton_kernel",
                "deep_gemm",
            ...
            assert resolved_view(self).moe_runner_backend in allowed_runners, (
                "moe_a2a_backend='ampere_ep' supports triton, triton_kernel, "
                "deep_gemm, or auto moe runner backends on SM80+"
            )
