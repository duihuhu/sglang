"""CPU-only regression tests for Qwen3 pure-AF FFN logits."""
...
class TestQwen3AFDFFNLogits(CustomTestCase):
    def _model(self, hidden_states, logits_output):
        ...
        model.lm_head = SimpleNamespace()  # FFN may not expose a loaded weight.
    ...
    def test_ffn_returns_vocab_sized_dummy_without_logits_processor(self):
        ...
        with mock.patch("sglang.srt.models.qwen3.afd_is_ffn", return_value=True):
            output = model(
                torch.tensor([1, 2, 3]),
                torch.tensor([0, 1, 2]),
                forward_batch,
    ...
    def test_attention_keeps_normal_logits_processor(self):
        ...
        with mock.patch("sglang.srt.models.qwen3.afd_is_ffn", return_value=False):
