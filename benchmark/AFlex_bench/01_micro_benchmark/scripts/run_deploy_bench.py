...
DEPLOYMENTS = {
    ...
    "pdaf_m2_dyn8": {
        "label": "PD+AF M=2 DynM thr=8 (PA7 PF6 DA5 DF4, each TP=1, 4 GPU)",
        "gpus": [4, 5, 6, 7], "prefill_gpus": [6, 7], "decode_gpus": [4, 5], "ngpu": 4,
        "dynamic_mb": True, "dynamic_mb_threshold": 8,
    },
    "pdaf_d_tp2": {
...
def _afd_cmd(port, perspective, disagg, tp, base_gpu_id, attn_tp=None, ffn_tp=None,
             ...
             dynamic_mb=False, dynamic_mb_threshold=None):
    ...
    if dynamic_mb:
        ...
        if dynamic_mb_threshold is not None:
            cmd += ["--afd-dynamic-mb-threshold", str(dynamic_mb_threshold)]
...
def _start_af_prefill(env_base, log_dir, prefix, procs, tier=False, stats_path=None,
                      p_vis="6,7", micro_batch=2, dynamic_mb=False, dynamic_mb_threshold=None):
    ...
    _popen("pf", _afd_cmd(PF_PORT, "ffn", "prefill", tp=1, base_gpu_id=0,
                          ...
                          dynamic_mb=dynamic_mb, dynamic_mb_threshold=dynamic_mb_threshold),
...
_DECODE_LAYOUT = {
    ...
    "pdaf_m2_dyn8": ("4,5",  1, 0, 1, 1, None, None, 1, 0),
...
def start_pdaf(deploy, log_dir, prefix):
    ...
    # dyn8 variants reuse m2 base layout
    if base.endswith("_dyn8"):
        base = base[:-5]  # pdaf_m2_dyn8 -> pdaf_m2
    ...
    dynamic_mb_threshold = spec.get("dynamic_mb_threshold", None)
