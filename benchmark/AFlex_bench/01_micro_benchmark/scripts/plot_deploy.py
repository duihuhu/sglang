...
def plot_tier(data):
    """11-way on il512: pure-PD vs bare-AF / AF+DVFS at M=1 and M=2,
    plus max-freq variants and DynM threshold=8."""
    import numpy as np
    grp = data.get(("512", "128"), {})
    order = ["pd_p2d2", "pd_p2d2_max", "pd_dp2", "pd_dp2_max",
             "pdaf_m2", "pdaf_m2_max", "pdaf_m2_dyn8", "pdaf_m2_dyn8_max",
             "pdaf_m2_tier",
             "pdaf_m1", "pdaf_m1_tier"]
    cols = {"pd_p2d2": "#8172B3", "pd_p2d2_max": "#5B4A99",
            "pd_dp2": "#C44E52", "pd_dp2_max": "#922A2E",
            "pdaf_m2": "#DA8BC3", "pdaf_m2_max": "#B85DA0",
            "pdaf_m2_dyn8": "#7FBF7F", "pdaf_m2_dyn8_max": "#3D8B3D",
            "pdaf_m2_tier": "#55A868",
            "pdaf_m1": "#DD8452", "pdaf_m1_tier": "#4C72B0"}
    labels = {"pd_p2d2": "PD-TP2 (auto)", "pd_p2d2_max": "PD-TP2 (max)",
              "pd_dp2": "PD-DP2 (auto)", "pd_dp2_max": "PD-DP2 (max)",
              "pdaf_m2": "AF M=2 (auto)", "pdaf_m2_max": "AF M=2 (max)",
              "pdaf_m2_dyn8": "AF DynM thr8 (auto)", "pdaf_m2_dyn8_max": "AF DynM thr8 (max)",
              "pdaf_m2_tier": "AF+DVFS M=2",
              "pdaf_m1": "AF M=1 (auto)", "pdaf_m1_tier": "AF+DVFS M=1"}
    ...
    for ax, (key, title) in zip(axes, panels):
        ...
        if key == "ttft_avg_ms":
            ax.set_yscale("log")
    fig.suptitle("PD (TP2/DP2) vs AF (M=1/M=2/DynM-thr8) — auto vs max vs DVFS "
                 "@ il512_ol128 (4 GPU)",
                 fontsize=12)
    fig.tight_layout()
    fig.savefig(FIG_DIR / "deploy_tier_tradeoff.png", dpi=120)
    plt.close(fig)
