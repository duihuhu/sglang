...
overlap_decode = ["--afd-enable-overlap-schedule"]
overlap_prefill = []
...
if os.path.exists(dump_file):
    ...
    log.info("=== %s ===", label)
    log.info("  Throughput: %.1f tok/s, TTFT: %.1f ms, TPOT: %.1f ms",
             throughput, sum(ttft)/len(ttft) if ttft else 0, sum(tpot)/len(tpot) if tpot else 0)
...
print("\n" + "=" * 100)
print("  FINAL 3-WAY COMPARISON (No shared prefix)")
print("=" * 100)
print(f"  {'Config':<35} {'Thru(tok/s)':<14} {'TTFT(ms)':<12} {'TPOT(ms)':<12} {'Success':<10}")
print("  " + "-" * 83)
for r in all_results:
    ttft_s = f"{r['mean_ttft_ms']:<12.1f}" if r['mean_ttft_ms'] else f"{'N/A':<12}"
    tpot_s = f"{r['mean_tpot_ms']:<12.1f}" if r['mean_tpot_ms'] else f"{'N/A':<12}"
    print(f"  {r['label']:<35} {r['throughput_tok_s']:<14.1f} {ttft_s} {tpot_s} {str(r['succeeded'])+'/'+str(r['total']):<10}")
print("=" * 100)
