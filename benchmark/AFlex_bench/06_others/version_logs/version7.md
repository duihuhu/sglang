...
今日进展：
1. **4-GPU/8-GPU TTFT 去排队延迟测试**：将 TTFT 指标从端到端改为纯处理时间（`ttft_proc_avg_ms`），去除排队干扰。4-GPU 结果：PD DP2 proc 最低（~49ms），PDAF DynM ~115ms，PDAF+Tier 因降频升至 256-706ms。8-GPU 结果：Tier 稳定节能 20-25% 且 SLO 违背 0%；PD DP4 在 overload 下吞吐最高（1004 tok/s）。
2. **Baseline 调频 3-way 对比**：在 4-GPU 上对 PDAF/PD DP2/Native DP4 三种架构实施 Tier DVFS，结果差异显著：PDAF 节能 29.2%，Native DP4 节能 36.9%，PD DP2 仅 0.6%（KV Transfer 瓶颈导致降频无效）。全部 SLO 违背 0%。

明日规划：
- 完成 Joint TTFT×TPOT SLO 二维热力图
- 分析 PD DP2 调频无效的根因
