#!/usr/bin/env python3
"""Joint TTFT x TPOT SLO sweep — NO-QUEUE TTFT version, 8x8 grid (GPU 4-7).

Extended grid (64 combos):
...
Reuses existing results from the 5x5 run where available.
"""
import json
import subprocess
import time
...
TTFT_SLOS = [1000, 500, 450, 400, 350, 300, 200, 100]
TPOT_SLOS = [200, 150, 140, 130, 120, 100, 90, 80]
...
def run_one(ttft: int, tpot: int, scheme: str):
    ...
    summary = out_dir / "4gpu_summary.json"
    if summary.exists():
        with open(summary) as f:
            sdata = json.load(f)
        for _dk, wl in sdata.items():
            if isinstance(wl, dict):
                for _wk, qps in wl.items():
                    if isinstance(qps, dict):
                        for _qk, r in qps.items():
                            if isinstance(r, dict) and r.get("throughput_tok_s", 0) > 0:
                                print(f"  [SKIP] {tag} (summary done)", flush=True)
                                return True
...
def main():
    ...
    print(f"Joint TTFT x TPOT SLO Sweep — NO-QUEUE 8x8 (GPU {GPU_BASE}-{GPU_BASE+3})")
