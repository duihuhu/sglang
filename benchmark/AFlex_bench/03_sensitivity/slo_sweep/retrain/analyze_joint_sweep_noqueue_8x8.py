#!/usr/bin/env python3
"""Analyze joint TTFT x TPOT SLO sweep — NO-QUEUE version, 8x8 grid.

Produces a 2x2 heatmap panel (8x8 cells each) from the extended sweep results.
Combines original 5x5 data with new data points.
"""
import json
...
def main():
    ...
    fig.suptitle("Joint TTFT x TPOT SLO Sweep (8x8) — V2 DVFS\n"
    ...
    def plot_heatmap(ax, data, title, cmap, vmin=None, vmax=None, fmt=".1f"):
        ...
        ax.set_xticklabels(tpot_labels, fontsize=9)
        ax.set_yticks(range(n_ttft))
        ax.set_yticklabels(ttft_labels, fontsize=9)
