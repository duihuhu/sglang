#!/usr/bin/env python3
"""
High-concurrency sweep v2: extend to higher batch sizes (2048-6144).
Also monitors decode-stage running batch size during benchmark.
"""
import json, logging, os, socket, subprocess, sys, time, urllib.request
import threading

logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] %(message)s")
log = logging.getLogger("high_conc_v2")
...
# ── Decode batch monitor ─────────────────────────────────────────────────

class DecodeBatchMonitor:
    """Poll decode server /v1/loads to track running batch size."""

    def __init__(self, decode_url, interval=0.5):
...
def main():
    ...
    for label, start_fn in configs:
        log.info("\n" + "=" * 80)
        log.info(">>> %s (high conc v2, in=%d out=%d) <<<", label, INPUT_LEN, OUTPUT_LEN)
        log.info("=" * 80)
    ...
    # Print summary
    print("\n" + "=" * 140)
    print(f"  High-Concurrency Sweep v2 ({CONCURRENCY_LEVELS[0]}-{CONCURRENCY_LEVELS[-1]}): in={INPUT_LEN}, out={OUTPUT_LEN}")
    print("=" * 140)
    print(f"  {'Conc':>5} | {'Config':<8} | {'TTFT(ms)':>9} | {'TPOT(ms)':>9} | {'Thru(t/s)':>10} | "
          f"{'D_max':>6} | {'D_mean':>7} | {'D_p95':>6} | {'OK':>10}")
    print("  " + "-" * 125)
        ...
        for label, _ in configs:
            ...
            if r and not r.get("error"):
                ...
                print(f"  {conc:>5} | {label:<8} | {r.get('mean_ttft_ms',0):>9.1f} | "
                      ...
                      f"{db.get('p95_running','?'):>6} | {r['n_ok']:>4}/{r['n_requests']}")
            elif r:
                db = r.get("decode_batch", {})
                print(f"  {conc:>5} | {label:<8} | {'FAILED':^30} | "
                      f"{db.get('max_running','?'):>6} | {db.get('mean_running','?'):>7} | "
                      f"{db.get('p95_running','?'):>6} |")
        if conc < CONCURRENCY_LEVELS[-1]:
            print("  " + "-" * 125)

    print("=" * 140)

    output_file = os.path.join(LOG_DIR, "high_conc_v2_results.json")
