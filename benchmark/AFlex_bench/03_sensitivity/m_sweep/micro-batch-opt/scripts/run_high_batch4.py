#!/usr/bin/env python3
"""High batch test with increased Mooncake transfer concurrency.

Goal: get decode batch to 256+ so M=2 splits into 2×128 (compute-bound).
- SGLANG_DISAGGREGATION_THREAD_POOL_SIZE=64
- SGLANG_DISAGGREGATION_QUEUE_SIZE=16
- output=512, conc=500 to let requests pile up in decode
"""
import asyncio, json, logging, os, re, socket, subprocess, sys, time, urllib.request

logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] %(message)s")
log = logging.getLogger("highbatch4")
...
def start_pdaf(micro_batch: int, async_pipeline: bool = False):
    ...
    # Increase Mooncake transfer concurrency
    env_base['SGLANG_DISAGGREGATION_THREAD_POOL_SIZE'] = '64'
    env_base['SGLANG_DISAGGREGATION_QUEUE_SIZE'] = '16'
    if async_pipeline:
    ...
    extra = [
        ...
        '--num-reserved-decode-tokens', '128',  # Reduce reservation to allow more concurrent reqs
    ]
    if async_pipeline:
    ...
    def _start(name, perspective, disagg_mode, port, ucx_base, sched_port,
        ...
        fh = open(os.path.join(HERE, f'hb4_{tag}_{name}.log'), 'w')
    ...
    rf = open(os.path.join(HERE, f'hb4_m{micro_batch}_router.log'), 'w')
...
def get_max_batch(da_log):
    """Extract max running-req from DA log."""
    max_batch = 0
...
def main():
    ...
    for label, m, async_pipe in configs:
        ...
        da_log = os.path.join(HERE, f'hb4_{tag}_da.log')
        max_batch = get_max_batch(da_log)

        if r:
            all_results[label] = {**r, 'max_batch': max_batch}
            log.info('%s: TPOT=%.1fms Thru=%.1f tok/s (%d/%d ok) MaxBatch=%d',
                     label, r['mean_tpot_ms'], r['throughput_tok_s'], r['n_ok'], r['n_total'], max_batch)
        else:
    ...
    # Summary
    print('\n' + '=' * 90)
    print(f'  High-Batch Results (out={OUTPUT_TOKENS}, conc={CONCURRENCY}, transfer_threads=64)')
    print('=' * 90)
    for label, _, _ in configs:
        r = all_results.get(label)
        if r:
            print(f'  {label:<15} TPOT={r["mean_tpot_ms"]:.1f}ms  p50={r["p50_tpot_ms"]:.1f}ms  Thru={r["throughput_tok_s"]:.1f} tok/s  MaxBatch={r["max_batch"]}  [{r["n_ok"]}/{r["n_total"]} ok]')
        else:
    ...
    with open(os.path.join(HERE, 'high_batch4_results.json'), 'w') as f:
