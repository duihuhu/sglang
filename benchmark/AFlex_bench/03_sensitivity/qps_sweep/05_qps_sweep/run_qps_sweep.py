#!/usr/bin/env python3
"""Run PD+AF M=1 at multiple QPS levels: 2, 4, 6, 8 req/s, 500 requests each."""
import json, logging, os, socket, subprocess, sys, time

logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] %(message)s")
log = logging.getLogger("qps_sweep")
...
INPUT_LEN = 512
ROUTER_PORT = 50000
CONCURRENCY = 500  # high enough to never throttle
...
extra = [
    ...
    "--afd-micro-batch", "1", "--max-running-requests", "500",
]
...
start_server("PF", pf_cmd, pf_env, "qps_sweep_pf.log")
...
start_server("PA", pa_cmd, pa_env, "qps_sweep_pa.log")
...
start_server("DF", df_cmd, df_env, "qps_sweep_df.log")
...
start_server("DA", da_cmd, da_env, "qps_sweep_da.log")
...
router_fh = open(os.path.join(LOG_DIR, "qps_sweep_router.log"), "w")
...
if not wait_health(f"http://127.0.0.1:{ROUTER_PORT}/health", timeout=120):
    ...
    sys.exit(1)
log.info("All M=1 servers ready")
...
# ── Final table ───────────────────────────────────────────────────────────
print("\n" + "=" * 90)
print("  PD+AF M=1 QPS Sweep — QPS 2,4,6,8")
...
# Save
with open(os.path.join(LOG_DIR, "qps_sweep_results.json"), "w") as f:
