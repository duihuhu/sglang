#!/usr/bin/env python3
"""Benchmark MegaScale (same solver topology as AFlex, but locked at max freq).

Same deployment layout as tier1_all_qps (solver-derived k_p/k_d/TP),
but all GPUs locked at 1410 MHz (no DVFS). This isolates the energy
savings from DVFS vs. topology optimization alone.
"""
...
log = logging.getLogger("bench_megascale_allqps")
...
# Same topology as AFlex solver configs, but will be run at max freq
CONFIGS = [
    Tier1TestConfig("mega_q2", qps=2, k_p=2, k_d=1, tp_pa=2, tp_pf=2, tp_da=1, tp_df=1,
                    f_pa=MAX_FREQ, f_pf=MAX_FREQ, f_da=MAX_FREQ, f_df=MAX_FREQ, tier=False),
    Tier1TestConfig("mega_q4", qps=4, k_p=4, k_d=1, tp_pa=1, tp_pf=1, tp_da=1, tp_df=1,
                    f_pa=MAX_FREQ, f_pf=MAX_FREQ, f_da=MAX_FREQ, f_df=MAX_FREQ, tier=False),
    Tier1TestConfig("mega_q6", qps=6, k_p=4, k_d=1, tp_pa=1, tp_pf=1, tp_da=1, tp_df=1,
                    f_pa=MAX_FREQ, f_pf=MAX_FREQ, f_da=MAX_FREQ, f_df=MAX_FREQ, tier=False),
    Tier1TestConfig("mega_q8", qps=8, k_p=4, k_d=1, tp_pa=2, tp_pf=1, tp_da=1, tp_df=1,
                    f_pa=MAX_FREQ, f_pf=MAX_FREQ, f_da=MAX_FREQ, f_df=MAX_FREQ, tier=False),
    Tier1TestConfig("mega_q12", qps=12, k_p=4, k_d=1, tp_pa=2, tp_pf=1, tp_da=1, tp_df=1,
                    f_pa=MAX_FREQ, f_pf=MAX_FREQ, f_da=MAX_FREQ, f_df=MAX_FREQ, tier=False),
    Tier1TestConfig("mega_q16", qps=16, k_p=4, k_d=1, tp_pa=2, tp_pf=1, tp_da=1, tp_df=1,
                    f_pa=MAX_FREQ, f_pf=MAX_FREQ, f_da=MAX_FREQ, f_df=MAX_FREQ, tier=False),
]
...
def main():
    ...
    targets = [c for c in CONFIGS if c.qps in qps_targets]

    all_results = {}
    for cfg in targets:
        gpu_count = cfg.k_p * (cfg.tp_pa + cfg.tp_pf) + cfg.k_d * (cfg.tp_da + cfg.tp_df)
        log.info("\n" + "#" * 72)
        log.info("MEGASCALE: %s (QPS=%d, %dGPU, all@%dMHz)", cfg.name, cfg.qps, gpu_count, MAX_FREQ)
        log.info("#" * 72)
        ...
        # Lock all GPUs to max freq (MegaScale = no DVFS)
        gpus = RMB.card_gpus(16)
        RMB.lock_freq_both(gpus, MAX_FREQ)
        log.info("  Locked all GPUs to %dMHz on both nodes", MAX_FREQ)
    ...
    out = RESULTS_DIR / f"megascale_allqps_{ts}.json"
    meta = {
        "benchmark": "megascale_all_qps_sweep",
        "dataset": DATASET,
        "ttft_slo_ms": RMB.TTFT_SLO_MS,
        "tpot_slo_ms": RMB.TPOT_SLO_MS,
        "freq_policy": f"locked_{MAX_FREQ}MHz",
    ...
    print(f"\n{'='*72}")
    print(f"MEGASCALE ALL-QPS RESULTS (code dataset, locked {MAX_FREQ}MHz)")
    print(f"{'='*72}")
    for cfg in targets:
