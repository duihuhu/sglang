...
ROUTER_PORT = 43000
PA_PORT, PF_PORT = 43010, 43012  # PF at +2 to avoid PA's auto-bootstrap port (43011)
...
def _afd_env(role, attn_gpus, ffn_gpus, ucx_port, sched_port, host: str):
    cvd = "0,1,2,3,4,5,6,7"
    ipc_sync = "ipc_event"
    base = (
...
def _afd_common(tp, tier: bool, *, is_decode: bool = False):
    # Prefill (PA/PF): lower fraction — KV is transient, more headroom for activations.
    # Decode (DA/DF): higher fraction — DA holds all in-flight decode KV.
    mem_frac = "0.88" if is_decode else "0.75"
    flags = (
        f"--model-path {RMB.MODEL} --tp {tp} --gpu-id-step 2 "
        "--afd-comm-backend ipc_cpp --afd-micro-batch 1 "
        f"--mem-fraction-static {mem_frac} --max-running-requests 512 "
    ...
    if tier:
        ...
        # Conservative prefill DVFS margin (env-tunable; 1.0 = disabled).
        import os as _os

        slack_factor = _os.environ.get("AFD_PREFILL_SLACK_FACTOR")
        if slack_factor:
            flags += f" --afd-dvfs-prefill-slack-factor {slack_factor}"
    return flags


def _lock_node_gpus(host: str, gpu_freq: dict[int, int]):
    RMB.lock_freq_map_on_host(host, gpu_freq)


def _launch(host, role, mode, attn, ffn, tp, port, base_gpu, cf, tag, ucx, sched, nccl_port=None):
    persp = "ffn" if role == "f" else "attn"
    env = _afd_env(persp, attn, ffn, ucx, sched, host)
    nccl_flag = f"--nccl-port {nccl_port}" if nccl_port else ""
    cmd = (
        f"{env} {RMB.PYTHON} -m sglang.launch_server --host {host} "
        f"--port {port} --afd-perspective {persp} "
        f"--disaggregation-mode {mode} --base-gpu-id {base_gpu} {nccl_flag} {cf}"
    )
...
def deploy_tier1_layout(cfg: Tier1Layout) -> Optional[str]:
    ...
    p_cf = _afd_common(cfg.tp_pa, cfg.tier, is_decode=False)
    d_cf = _afd_common(cfg.tp_da, cfg.tier, is_decode=True)

    dec_placements = _plan_decode_placements(cfg.k_d, cfg.tp_da, p_attn, p_ffn)

    # Lock per-GPU frequencies before launch (AFlex tier initial state).
    # MegaScale locks once in harness after servers are up.
    ...
    for pl in dec_placements:
        ...
        df_port = DECODE_PORT_BASE + pl.idx * 10 + 2  # +2 to avoid DA's bootstrap port
    ...
    checks = [
        ...
        (RMB.NODE1_IP, PA_PORT, True, "PA"),
    ]
    for host, da_port, name in decode_endpoints:
        checks.append((host, da_port, True, name))
    for host, port, mi, name in checks:
        timeout = 600 if host != RMB.NODE1_IP else 300
        if not RMB.wait_health(host, port, timeout, check_model_info=mi):
    ...
    for host, da_port, _ in decode_endpoints:
        ...
        rc = (
            ...
            f"--prefill http://{RMB.NODE1_IP}:{PA_PORT} {PA_PORT + 1} "
    ...
    RMB.dexec_local(rc)
    if not RMB.wait_health(RMB.NODE1_IP, ROUTER_PORT, 120):
        return None
    # Allow remote decode JIT / mooncake handshake to settle before first generate.
    time.sleep(10)
