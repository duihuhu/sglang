...
def _run_in_container() -> bool:
    """True when the orchestrator runs inside operator_test (not on host)."""
    flag = os.environ.get("MN_RUN_IN_CONTAINER")
    if flag is not None and flag.strip().lower() in ("0", "false", "no"):
        return False
    if flag is not None and flag.strip().lower() in ("1", "true", "yes"):
        return True
    return Path("/.dockerenv").exists()


def _node1_container_exec(shell_cmd: str, **kwargs):
    """Run a shell command in node1's operator_test workload environment."""
    _ensure_local_node1_flag()
    if _LOCAL_NODE1 and _run_in_container():
        return subprocess.run(["bash", "-lc", shell_cmd], **kwargs)
    if _LOCAL_NODE1:
        return subprocess.run(
            ["docker", "exec", CONTAINER, "bash", "-lc", shell_cmd], **kwargs
        )
...
def sync_dvfs_py_to_host(host: str) -> None:
    """Copy dvfs.py into operator_test on host (node1 local docker or SSH)."""
    if not DVFS_PY_SRC.exists():
        raise FileNotFoundError(f"dvfs.py not found: {DVFS_PY_SRC}")
    _ensure_local_node1_flag()
    if host == NODE1_IP and _LOCAL_NODE1 and _run_in_container():
        return
    tmp = "/tmp/sglang_dvfs.py"
    if host == NODE1_IP and _LOCAL_NODE1:
        subprocess.run(
            ["docker", "cp", str(DVFS_PY_SRC), f"{CONTAINER}:{DVFS_PY_CONTAINER}"],
            check=True,
        )
    else:
        subprocess.run(
            ["scp", "-o", "StrictHostKeyChecking=no", "-q", str(DVFS_PY_SRC), f"root@{host}:{tmp}"],
            check=True,
        )
        subprocess.run(
            _ssh(host, f"docker cp {tmp} {CONTAINER}:{DVFS_PY_CONTAINER}"),
            check=True,
        )
    log.info("  Synced dvfs.py -> %s:%s", host, DVFS_PY_CONTAINER)


def sync_dvfs_py_both() -> None:
    sync_dvfs_py_to_host(NODE1_IP)
    sync_dvfs_py_to_host(NODE2_IP)
    if NODE_EXTRA_IP:
        sync_dvfs_py_to_host(NODE_EXTRA_IP)


def _all_remote_hosts() -> list[str]:
    hosts = [NODE2_IP]
    if NODE_EXTRA_IP:
        hosts.append(NODE_EXTRA_IP)
    return hosts


def dexec_on_host(host: str, shell_cmd: str) -> None:
    """Run shell inside operator_test on an arbitrary cluster node."""
    _ensure_local_node1_flag()
    if host == NODE1_IP and _LOCAL_NODE1:
        _node1_container_exec(shell_cmd, check=False)
        return
    inner = f"docker exec {CONTAINER} bash -lc {shlex.quote(shell_cmd)}"
...
def dexec_remote(shell_cmd):
    dexec_on_host(NODE2_IP, shell_cmd)
...
def cleanup_all():
    ...
    for host in _all_remote_hosts():
        subprocess.run(_ssh(host, f"docker exec {CONTAINER} bash -lc 'bash {CLEANUP}'"),
                       check=False)
    time.sleep(5)
    # Verify critical AFD ports are released; retry cleanup if still occupied
    for attempt in range(3):
        occupied = []
        for host in (NODE1_IP, *_all_remote_hosts()):
            check_cmd = (
                "ss -tlnp 2>/dev/null | grep -v '127.0.0.1' "
                "| grep -oP ':(43[0-9]{3}|45[0-9]{3})' | sort -u || true"
            )
            try:
                if host == NODE1_IP and _LOCAL_NODE1:
                    out = _node1_container_exec(
                        check_cmd, capture_output=True, text=True, timeout=10
                    )
                else:
                    inner = f"docker exec {CONTAINER} bash -lc {shlex.quote(check_cmd)}"
                    out = subprocess.run(
                        _ssh(host, inner), capture_output=True, text=True, timeout=10
                    )
                if out.stdout.strip():
                    occupied.append(f"{host}: {out.stdout.strip()}")
            except Exception:
                pass
        if not occupied:
            break
        log.warning("Ports still occupied after cleanup (attempt %d): %s", attempt + 1, occupied)
        _kill_stale_benchmark_ports(NODE1_IP, NODE2_IP)
        dexec_local(f"bash {CLEANUP}")
        for host in _all_remote_hosts():
            subprocess.run(_ssh(host, f"docker exec {CONTAINER} bash -lc 'bash {CLEANUP}'"),
                           check=False)
...
def _kill_ports_on_host(host: str, ports: list[str]) -> None:
    """Force-kill listeners on specific TCP ports inside operator_test."""
    if not ports:
        return
    port_csv = " ".join(str(int(p.lstrip(":"))) for p in ports)
    kill_cmd = (
        f"for p in {port_csv}; do "
        f"for pid in $(ss -tlnp sport = :$p 2>/dev/null | grep -oP 'pid=\\K[0-9]+'); do "
        f"kill -9 $pid 2>/dev/null; done; "
        f"fuser -k ${{p}}/tcp 2>/dev/null; done"
    )
    _ensure_local_node1_flag()
    if host == NODE1_IP and _LOCAL_NODE1:
        _node1_container_exec(kill_cmd, check=False)
    else:
        inner = f"docker exec {CONTAINER} bash -lc {shlex.quote(kill_cmd)}"
        subprocess.run(_ssh(host, inner), check=False)


def _kill_stale_benchmark_ports(node1: str, node2: str) -> None:
    """Kill Tier1/AFlex stale ports detected by cleanup_all (e.g. :43221)."""
    check_cmd = (
        "ss -tlnp 2>/dev/null | grep -oP ':\\K(43[0-9]{3}|45[0-9]{3})' | sort -u || true"
    )
    for host in (node1, node2):
        try:
            if host == node1 and _LOCAL_NODE1:
                out = _node1_container_exec(
                    check_cmd, capture_output=True, text=True, timeout=10
                )
            else:
            ...
            ports = [p.strip() for p in out.stdout.split() if p.strip()]
            if ports:
                log.info("  Force-killing stale ports on %s: %s", host, ",".join(ports))
                _kill_ports_on_host(host, ports)
        except Exception as exc:
            log.warning("  Stale port cleanup failed on %s: %s", host, exc)
...
def get_energy_remote(gpus):
    return get_energy_on_host(NODE2_IP, gpus)


def get_energy_on_host(host: str, gpus):
    if not gpus:
        return {}
    idx_csv = ",".join(str(i) for i in gpus)
    pycode = (
    ...
    try:
        if host == NODE1_IP:
            _ensure_local_node1_flag()
            if _LOCAL_NODE1:
                out = _node1_container_exec(inner, capture_output=True, text=True, timeout=30)
            else:
                out = subprocess.run(_ssh(host, inner), capture_output=True,
                                     text=True, timeout=30)
        else:
        ...
        log.warning("NVML energy read (%s) failed: %s", host, e)
        return {i: 0 for i in gpus}


def _dvfs_py_snippet_lock(gpus: list[int], freq: int) -> str:
    idx_csv = ",".join(str(i) for i in gpus)
    return (
        "from sglang.srt.layers.dvfs import lock_gpus; "
        f"n=lock_gpus([{idx_csv}], {freq}); "
        "import sys; sys.exit(1 if n else 0)"
    )


def _dvfs_py_snippet_unlock(gpus: list[int]) -> str:
    idx_csv = ",".join(str(i) for i in gpus)
    return (
        "from sglang.srt.layers.dvfs import unlock_gpus; "
        f"n=unlock_gpus([{idx_csv}]); "
        "import sys; sys.exit(1 if n else 0)"
    )


def _dvfs_py_snippet_lock_map(gpu_freq: dict[int, int]) -> str:
    mapping = repr(dict(sorted(gpu_freq.items())))
    return (
        "from sglang.srt.layers.dvfs import lock_gpu_freq_map; "
        f"n=lock_gpu_freq_map({mapping}); "
        "import sys; sys.exit(1 if n else 0)"
    )


def _dvfs_exec_local(py_snippet: str) -> None:
    shell = f"{PYTHON} -c {shlex.quote(py_snippet)}"
    _ensure_local_node1_flag()
    ret = _node1_container_exec(shell, capture_output=True, text=True)
    if ret.returncode != 0:
        err = (ret.stderr or ret.stdout or "").strip()
        raise RuntimeError(f"DVFS exec failed on {NODE1_IP}: {err}")


def _dvfs_exec_remote(host: str, py_snippet: str) -> None:
    shell = f"{PYTHON} -c {shlex.quote(py_snippet)}"
    inner = f"docker exec {CONTAINER} bash -lc {shlex.quote(shell)}"
    ret = subprocess.run(_ssh(host, inner), capture_output=True, text=True)
    if ret.returncode != 0:
        err = (ret.stderr or ret.stdout or "").strip()
        raise RuntimeError(f"DVFS exec failed on {host}: {err}")


def lock_freq_both(gpus, freq=MAX_GPU_FREQ):
    """Lock GPUs on both nodes via dvfs.py (SetGpuLockedClocks)."""
    sync_dvfs_py_both()
    py = _dvfs_py_snippet_lock(gpus, freq)
    _dvfs_exec_local(py)
    _dvfs_exec_remote(NODE2_IP, py)
    log.info("  Locked GPUs %s to %d MHz on both nodes (dvfs.py)", gpus, freq)


def unlock_freq_both(gpus):
    """Unlock GPUs on both nodes via dvfs.py."""
    sync_dvfs_py_both()
    py = _dvfs_py_snippet_unlock(gpus)
    _dvfs_exec_local(py)
    _dvfs_exec_remote(NODE2_IP, py)
    log.info("  Unlocked GPUs %s on both nodes (dvfs.py)", gpus)


def lock_freq_map_on_host(host: str, gpu_freq: dict[int, int]) -> None:
    """Lock per-GPU frequencies on one node via dvfs.py inside the container."""
    if not gpu_freq:
        return
    sync_dvfs_py_to_host(host)
    py = _dvfs_py_snippet_lock_map(gpu_freq)
    shell = f"{PYTHON} -c {shlex.quote(py)}"
    _ensure_local_node1_flag()
    if host == NODE1_IP and _LOCAL_NODE1:
        ret = _node1_container_exec(shell, capture_output=True, text=True)
    else:
    ...
    if ret.returncode != 0:
        err = (ret.stderr or ret.stdout or "").strip()
        raise RuntimeError(f"DVFS lock map failed on {host}: {err}")
...
def write_ib_json(mapping):
    ...
    with tempfile.NamedTemporaryFile(mode="w", suffix=".json", delete=False) as f:
        ...
        tmp = f.name
    if _LOCAL_NODE1 and _run_in_container():
        Path(IB_JSON_FILE).write_text(json.dumps(mapping))
    elif _LOCAL_NODE1:
    ...
    if NODE_EXTRA_IP:
        subprocess.run(["scp", "-o", "StrictHostKeyChecking=no", "-q",
                        tmp, f"{NODE_EXTRA_IP}:{IB_JSON_FILE}"], check=False)
        subprocess.run(_ssh(NODE_EXTRA_IP,
                            f"docker cp {IB_JSON_FILE} {CONTAINER}:{IB_JSON_FILE}"),
                       check=False)
    os.unlink(tmp)
...
def start_pd_hetero(ngpu, tier=False):
    """Heterogeneous PD: P-node 2xTP4 prefill + D-node 4xTP2 decode.

    16-card layout:
        node1 (Prefill):  2 TP4 instances [0,1,2,3] [4,5,6,7]
        node2 (Decode):   4 TP2 instances [0,1] [2,3] [4,5] [6,7]
    """
    log.info("PD Hetero %d-card: P=2xTP4@node1, D=4xTP2@node2, tier=%s", ngpu, tier)
    ...
    for inst in p_insts:
        ...
        cmd = (f"{_plain_env_multi(inst['gpus'])} setsid prlimit "
               ...
               f"--model-path {MODEL} --tp 4 --host {NODE1_IP} "
    ...
    for inst in d_insts:
        ...
        cmd = (f"{_plain_env_multi(inst['gpus'])} setsid prlimit "
               f"--memlock=unlimited:unlimited {PYTHON} -m sglang.launch_server "
               f"--model-path {MODEL} --tp 2 --host {NODE2_IP} "
    ...
    for inst in d_insts:
        if not wait_health(NODE2_IP, inst["port"], 500):
            ...
            return None
    log.info("  all PD-Het instances ready (2P + 4D)")
...
async def send_one(session, url, req, base_time, results):
    ...
    prompt = ({"input_ids": [1000] * req["input_len"]}
              if req.get("use_input_ids") else
