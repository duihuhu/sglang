#!/usr/bin/env python3
"""AFlex 13P(TP1)+2D(TP1) on 4 nodes (30 active GPU), code QPS32.

Each P/D instance = PA+PF or DA+DF = 2 GPUs at TP1.
  - n1,n2,n3: 4 P each  -> 12 P  (24 GPU)
  - n4:       1 P + 2 D  -> 13 P + 2 D (6 GPU active, GPU 6-7 idle)
Energy counted on 30 active GPUs only (excludes n4 GPU 6,7).
"""
...
OUT = ROOT / "results" / "aflex_13p2d_code_qps32_4node.json"
...
log = logging.getLogger("aflex_13p2d_4n")
...
QPS = 32
ACTIVE_GPUS = 30
# Energy: only the 30 GPUs used by 13P+2D (n4 GPU 6,7 are idle)
...
BT2.NCCL_PORT_BASE = 37300 + 2000

CFG = BT2.Tier1TestConfig(
    name="aflex_13p2d_code_qps32",
    k_p=13,
    k_d=2,
    tp_pa=1,
    tp_pf=1,
...
def plan_allocation_13p2d_4node(cfg: BT2.Tier1TestConfig) -> dict:
    """n1-n3: 4 P each; n4: 2 D (GPU 0-3) + 1 P (GPU 4-5)."""
    ...
    # n4: single P on GPU pair (4,5)
    attn, ffn = _pair_gpus(2)
...
def kill_stale_ports() -> None:
    ...
    new_ports = (
        ...
        + list(range(BT2.PREFILL_PORT_BASE, BT2.PREFILL_PORT_BASE + 140, 2))
...
def log_allocation(alloc: dict) -> None:
    log.info("Layout 13P(TP1)+2D(TP1): %d active GPU (energy on 30)", alloc["total_gpu"])
...
async def run_workload(urls: list[str], reqs: list[dict], run_s: int) -> dict:
    ...
    return {
        ...
        "nodes": 4,
        "gpus": ACTIVE_GPUS,
        "topology": "n1-n3:12P(4/node), n4:1P+2D",
        "k_p": CFG.k_p,
        "k_d": CFG.k_d,
...
def main() -> None:
    BT2.DATASET = DATASET
    BT2.QPS = QPS
    BT2.plan_allocation = plan_allocation_13p2d_4node  # type: ignore[method-assign]

    wl = WL_DIR / f"macro_{DATASET}_qps{QPS}.jsonl"
    if not wl.exists():
        raise FileNotFoundError(wl)

    alloc = plan_allocation_13p2d_4node(CFG)
    log_allocation(alloc)

    payload = {
        "meta": {
            "benchmark": "aflex_13p2d_code_qps32_4node",
            "topology": "n1-n3:12P(4/node), n4:1P+2D",
            "nodes": 4,
            "gpus": ACTIVE_GPUS,
            "energy_gpus": 30,
