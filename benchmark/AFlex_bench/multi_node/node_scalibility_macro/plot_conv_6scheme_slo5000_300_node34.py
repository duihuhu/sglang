#!/usr/bin/env python3
"""Plot conv 6-scheme dashboard from the node3+node4 results (pd_hetero D=4xTP2)."""
...
SCHEME_LABELS = {
    ...
    "pd_hetero_baseline": "DistServe (4×TP2)",
    "pd_hetero_tier": "BiScale (4×TP2)",
...
def load_results():
    files = sorted(RESULTS_DIR.glob("conv_6scheme_slo5000_300_node34_*.json"))
    if not files:
        raise FileNotFoundError("No node34 conv result file found")
    latest = files[-1]
...
def main():
    ...
    fig.suptitle(
        ...
        "DistServe/BiScale: P=4×TP2, D=4×TP2",
    ...
    out_path = CHARTS_DIR / "conv_6scheme_slo5000_300_node34_dashboard.png"
