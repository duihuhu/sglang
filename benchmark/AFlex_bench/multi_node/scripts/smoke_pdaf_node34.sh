#!/bin/bash
# 16-card PDAF smoke: node3 (prefill) + node4 (decode)
set -u

IB_DEV="${1:-mlx5_bond_0}"
TP="${2:-4}"
PREFILL_IP="10.252.129.34"   # node3
DECODE_IP="10.252.129.33"    # node4
...
if echo "$RESP" | grep -q '"text"'; then
  echo "=== PDAF NODE34 PASS (prefill=node3 decode=node4 ib=$IB_DEV tp=$TP) ==="
else
  echo "=== PDAF NODE34 FAIL ==="
