...
def main(argv=None):
 p=argparse.ArgumentParser();p.add_argument("--results",type=Path,default=ROOT/"results/raw/measurement_rdma2n_4gpu_qps4_20260825");a=p.parse_args(argv);progress=json.loads((a.results/"progress.json").read_text());groups=defaultdict(list);artifacts=defaultdict(list)
 ...
 out=a.results/("measurement_rdma2n_4gpu_report.json" if complete else "measurement_rdma2n_4gpu_interim.json");r2.atomic_write(out,report);print(json.dumps({"complete":complete,"valid":report["valid_runs"],"output":str(out)},indent=2));return 0 if complete else 3
