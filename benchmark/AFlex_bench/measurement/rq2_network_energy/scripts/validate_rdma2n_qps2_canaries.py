...
def main(argv=None):
 ...
 p.add_argument("--results",type=Path,default=ROOT/"results/raw/measurement_rdma2n_qps2_20260825");p.add_argument("--execute",action="store_true");a=p.parse_args(argv);b=r2.load_bundle_2n()
 if not a.execute:
 ...
 gate={"schema_version":2,"status":"pass","generated_at":now(),"generator_pid":os.getpid(),"topology":{"pairs":["pair34"],"lanes":[0,2,4,6],"total_gpus_per_job":2},"bindings":r2.hashes(b),"artifacts":records};r2.atomic_write(a.results/r2.GATE_NAME,gate);r2.atomic_write(a.results/"canary/canary.final.json",gate);print(json.dumps(gate,indent=2));return 0
