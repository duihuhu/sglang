...
import argparse,copy,fcntl,hashlib,json,os,re,signal,sys,threading,time
...
PAIRS={"pair34":("node3","node4")};SLOTS={"slotA":((0,2),("mlx5_0","mlx5_1")),"slotB":((4,6),("mlx5_4","mlx5_5"))};CANARY_PORT_START=17000;CANARY_JOB_COUNT=3;FORMAL_PORT_START=20000;FORMAL_PORT_STRIDE=48;FORMAL_PORT_ATTEMPTS=12;FORMAL_JOB_COUNT=54;DERIVED_PORT_MAX_OFFSET=41;SYSTEM_RESERVED_PORTS={16000};OCCUPIED_PORTS=set(SYSTEM_RESERVED_PORTS);GATE_NAME="rdma2n_4gpu_qps4_canary_gate.json";STATES={"preflight","canary","formal","paused","complete","failed"};CONFIG=ROOT/"configs/measurement_rdma2n_4gpu_qps4_qwen3_30b_a3b.json";MUTEX=threading.Lock();STOP=threading.Event()
...
def load_bundle_2n():
 ...
 return b
def resource_plan(c=2):return {"pairs":PAIRS,"slots":SLOTS,"max_concurrency":c,"planned_nodes":["node3","node4"],"excluded_nodes":["node1","node2"],"atomic_lease":["gpu_pair","hca_pair","ports"],"total_gpus_per_job":4}
def hashes(bundle,c=2):return {"config_hash":file_hash(CONFIG),"runtime_manifest_hash":digest({k:bundle["cluster"].get(k) for k in ("container","python_source","host_network")}),"resource_plan_hash":digest(resource_plan(c))}
...
def artifact_hash(path):
 ...
 return h.hexdigest()
def layout(pair,slot):return {n:list(SLOTS[slot][0]) for n in PAIRS[pair]}
...
def mapping(r,w,a):return "pair34",tuple(SLOTS)[(w*3+a+r-1)%2]
def logical_key(r,a,w):return f"{r}:{a}:{w}"
def allocate_port_base(ordinal,attempt,canary=False):
 if canary:
  if not 1<=ordinal<=CANARY_JOB_COUNT:raise ValueError(f"canary ordinal outside 1..{CANARY_JOB_COUNT}: {ordinal}")
  if not 1<=attempt<=FORMAL_PORT_ATTEMPTS:raise ValueError(f"attempt outside allocated range 1..{FORMAL_PORT_ATTEMPTS}: {attempt}")
  return CANARY_PORT_START+((attempt-1)*CANARY_JOB_COUNT+ordinal-1)*FORMAL_PORT_STRIDE
 if not 1<=ordinal<=FORMAL_JOB_COUNT:raise ValueError(f"ordinal outside 1..{FORMAL_JOB_COUNT}: {ordinal}")
 if not 1<=attempt<=FORMAL_PORT_ATTEMPTS:raise ValueError(f"attempt outside allocated range 1..{FORMAL_PORT_ATTEMPTS}: {attempt}")
 target=(attempt-1)*FORMAL_JOB_COUNT+ordinal-1;free_index=-1;slot=-1
 while free_index<target:
  slot+=1;base=FORMAL_PORT_START+slot*FORMAL_PORT_STRIDE
  if base+DERIVED_PORT_MAX_OFFSET>65535:raise ValueError("exhausted globally unique formal port ranges")
  if any(base<=port<=base+DERIVED_PORT_MAX_OFFSET for port in OCCUPIED_PORTS):continue
  free_index+=1
 return FORMAL_PORT_START+slot*FORMAL_PORT_STRIDE
def configure(base,repeat,ordinal,pair,slot,corunners,attempt,canary=False,config_hash=None):
 item=copy.deepcopy(base);p=item["point"];placed=layout(pair,slot);p.update(placement_fields(p["architecture"],placed));p["port_base"]=allocate_port_base(ordinal,attempt,canary);slot_id=f"{pair}:{slot}:gpu{'-'.join(map(str,SLOTS[slot][0]))}:hca{'-'.join(SLOTS[slot][1])}";identity={"repeat":repeat,"attempt":attempt,"point":p["id"],"workload":item["workload"],"qps":item["qps"],"resource_slot":slot_id,"config_hash":config_hash or file_hash(CONFIG),"canary":canary};item["run_id"]=digest(identity)[:24];actual={n:{"gpus":g,"nics":list(SLOTS[slot][1])} for n,g in placed.items()};p.setdefault("metadata",{}).update({"measurement_repeat":repeat,"retry_attempt":attempt,"logical_key":logical_key(repeat,p["architecture"],item["workload"]),"resource_slot":slot_id,"slot":slot,"gpu_pair":list(SLOTS[slot][0]),"hca_group":list(SLOTS[slot][1]),"actual_layout":actual,"concurrent_job_ids":list(corunners),"startup_barrier":"AFLEX_RUN_ID+role+port+environment+GPU_UUID","run_identity":identity,"canary":canary});return item,actual
...
def validate_plan_shape(bundle,item):
 ...
 if set(used)!={"node3","node4"} or used!={n:v["gpus"] for n,v in actual.items()} or sum(map(len,used.values()))!=4:raise ValueError(f"invalid total-4-GPU plan {used}")
 by={}
 for x in plan.processes:
  ...
  for token in (",".join(p["metadata"]["hca_group"]),f"CUDA_VISIBLE_DEVICES={','.join(map(str,x.gpus))}",f"AFLEX_RUN_ID={item['run_id']}",f"AFLEX_COMPONENT_ROLE={x.role}",f"AFLEX_COMPONENT_PORT={x.port}"):
 ...
 tp=4 if p["architecture"]=="native" else 2
 if any(f"--tp {tp}" not in x.command for xs in by.values() for x in xs):raise ValueError("wrong TP")
 if any(len(x.gpus)!=2 for xs in by.values() for x in xs):raise ValueError("every component process must own two GPUs")
 ...
 expected_nodes={"native":{"NATIVE":["node3","node4"]},"pd":{"P":["node3"],"D":["node4"]},"af":{"F":["node3"],"A":["node4"]}}[p["architecture"]]
 if {k:[x.node for x in v] for k,v in by.items()}!=expected_nodes:raise ValueError(f"wrong role placement {by}")
 return plan
def build_inventory(bundle,c=2):
 q=expand_queue(bundle);base={(x["point"]["architecture"],x["workload"]):x for x in q}
 if len(base)!=18 or any(x["qps"]!=4 for x in q):raise ValueError("expected 18 QPS4 base items")
 jobs=[];ids={};arts={};ports={};ordinal=0
 for repeat in (1,2,3):
  for wi,w in enumerate(WORKLOADS):
   for ai,a in enumerate(ARCHITECTURES):
    ordinal+=1;pair,slot=mapping(repeat,wi,ai);item,actual=configure(base[a,w],repeat,ordinal,pair,slot,[],1);plan=validate_plan_shape(bundle,item);key=logical_key(repeat,a,w);art=f"repeat{repeat}/attempt1/{item['run_id']}"
    ...
    ids[item["run_id"]]=key;arts[art]=key;lease=(pair,slot);attempt_ports={}
    for retry_attempt in range(1,FORMAL_PORT_ATTEMPTS+1):
     retry_item,_=configure(base[a,w],repeat,ordinal,pair,slot,[],retry_attempt);retry_plan=validate_plan_shape(bundle,retry_item);ps=plan_ports(retry_plan)
    ...
    jobs.append({"logical_id":key,"repeat":repeat,"architecture":a,"workload":w,"ordinal":ordinal,"pair":pair,"slot":slot,"gpu_pair":list(SLOTS[slot][0]),"hcas":list(SLOTS[slot][1]),"run_id_attempt1":item["run_id"],"ports":attempt_ports["1"],"port_leases":attempt_ports,"actual_layout":actual})
...
def strict_audit(path,item=None,actual=None):
 ...
 if len(flat)!=4 or len(set(flat))!=4 or any(not str(u).startswith("GPU-") for u in flat):return False,"energy_uuid_scope"
...
def require_gate(results,bundle=None,c=2):
 ...
 if g.get("status")!="pass" or g.get("topology")!={"pairs":["pair34"],"slots":["slotA","slotB"],"total_gpus_per_job":4}:raise RuntimeError("gate topology invalid")
 a=g.get("artifacts",[])
 ...
 expected=hashes(bundle or load_bundle_2n(),c)
 ...
 return g
def offline_preflight(results,c=2):
...
def online_preflight(results,c=2):
 b=load_bundle_2n();ex=RemoteExecutor(b["cluster"]["container"],False);block=[];discovered=set(SYSTEM_RESERVED_PORTS)
 for n in b["cluster"]["nodes"]:
  ...
  listeners=ex.host_run(n["host"],"ss -H -ltn",check=False,quiet=True)
  if listeners.returncode:block.append(f"{n['name']}:ports:{(listeners.stderr or listeners.stdout or 'failed').strip()}")
  else:discovered.update(int(x) for x in re.findall(r":(\d+)\s",listeners.stdout or ""))
 OCCUPIED_PORTS.clear();OCCUPIED_PORTS.update(discovered)
 out=offline_preflight(results,c);block.extend(out["blockers"])
 return {**out,"status":"pass" if not block else "blocked","mode":"online","ssh_performed":True,"blockers":block,"occupied_ports":sorted(discovered)}
...
def run_scheduler(a,b,inv):
 ...
 for j in inv["jobs"]:
  ...
  if a.only_logical_key and j["logical_id"] not in a.only_logical_key:continue
...
def parse_args(argv=None):
 p=argparse.ArgumentParser(description="2N 4GPU QPS4 runner; default is local dry-run with no SSH")
 p.add_argument("--results",type=Path,default=ROOT/"results/raw/measurement_rdma2n_4gpu_qps4_20260825");p.add_argument("--execute",action="store_true");p.add_argument("--resume",action="store_true");p.add_argument("--retry-blocked",action="store_true");p.add_argument("--max-attempts",type=int,default=3,help="additional attempts allowed for each selected key in this resume invocation");p.add_argument("--max-concurrency",type=int,choices=(1,2),default=2);p.add_argument("--only-logical-key",action="append",help="retry only an exact repeat:architecture:workload key; repeatable");p.add_argument("--backoff-base",type=float,default=5);p.add_argument("--inventory-json",type=Path);p.add_argument("--offline-preflight",action="store_true");p.add_argument("--preflight-only",action="store_true",help="online read-only checks; requires --execute");p.add_argument("--check-gate",action="store_true");return p.parse_args(argv)
def main(argv=None):
 ...
 lock=acquire(a.results,"formal");require_gate(a.results,b,2);pre=online_preflight(a.results,a.max_concurrency)
