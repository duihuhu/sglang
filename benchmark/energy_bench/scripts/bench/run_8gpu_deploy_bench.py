...
Tests 5 deployment topologies on all 8 A800 GPUs with 4 workload length
combinations at increasing QPS to find throughput ceilings and SLO collapse
points.

Topologies:
  - pd_p4d4       : Standard PD, Prefill TP4 + Decode TP4 (8 GPU)
  - pdaf_8g_m1    : PD+AF M=1, PF/PA/DF/DA each TP2 (8 GPU)
  - pdaf_8g_m2    : PD+AF M=2, PF/PA/DF/DA each TP2 (8 GPU)
  - pdaf_8g_m1_tier : PD+AF M=1 + Tier1/DVFS (8 GPU)
  - pdaf_8g_m2_tier : PD+AF M=2 + Tier1/DVFS (8 GPU)

Usage:
    python run_8gpu_deploy_bench.py --deploys pd_p4d4,pdaf_8g_m1,pdaf_8g_m2 \
...
DEPLOYMENTS = {
    "pd_p4d4": {
        "label": "PD P-tp4/D-tp4 (P=4,5,6,7 D=0,1,2,3, 8 GPU)",
        "gpus": list(range(8)),
    ...
    "pdaf_8g_m1": {
        "label": "PD+AF M=1 (PA/PF TP2 + DA/DF TP2, P=0,1,2,3 D=4,5,6,7, 8 GPU)",
        "gpus": list(range(8)),
    ...
    "pdaf_8g_m2": {
        "label": "PD+AF M=2 (PA/PF TP2 + DA/DF TP2, P=0,1,2,3 D=4,5,6,7, 8 GPU)",
        "gpus": list(range(8)),
        "prefill_gpus": [0, 1, 2, 3],
        "decode_gpus": [4, 5, 6, 7],
        "ngpu": 8,
    ...
    "pdaf_8g_m1_tier": {
        "label": "PD+AF M=1 + Tier1/DVFS (PA/PF TP2 + DA/DF TP2, P=0,1,2,3 D=4,5,6,7, 8 GPU)",
        "gpus": list(range(8)),
        "prefill_gpus": [0, 1, 2, 3],
        "decode_gpus": [4, 5, 6, 7],
        "ngpu": 8,
    ...
    "pdaf_8g_m2_tier": {
        "label": "PD+AF M=2 + Tier1/DVFS (PA/PF TP2 + DA/DF TP2, P=0,1,2,3 D=4,5,6,7, 8 GPU)",
        "gpus": list(range(8)),
        "prefill_gpus": [0, 1, 2, 3],
        "decode_gpus": [4, 5, 6, 7],
        "ngpu": 8,
...
def main():
    ...
    for g in wl_groups:
        wl_groups[g].sort(key=lambda x: float(x[1]["qps"]))
