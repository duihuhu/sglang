#!/usr/bin/env python3
"""Test M=1 vs M=3 with longer sequences where M=3 pipeline should be more effective.

Hypothesis: With longer seq_len, attn time grows (memory-bound, proportional to seq_len)
while FFN time stays constant. This means:
- M=1: recv_wait = FFN_time (constant) while A_stage grows → recv_wait becomes smaller fraction
- M=3: 3×A_stage grows, but recv_wait is already near 0 due to pipeline overlap

Actually this means M=1 gets BETTER with longer sequences (recv_wait fraction decreases).
So let's test with SHORTER sequences where recv_wait dominates.
"""
import json, logging, os, socket, subprocess, sys, time

logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] %(message)s")
log = logging.getLogger("seq_test")
...
def run_config(label, afd_args, max_running, concurrency, max_requests, input_len, output_len):
    kill_all()
    procs = []
...
# With short input (128 tokens), KV cache is small, attn is fast,
# and communication overhead dominates → M=3 pipeline should help more
configs = [
    # Short sequences (input=128): comm overhead dominates
    ("m1_s128", ["--afd-micro-batch", "1"], 64, 64, 128, 128, 128),
    ("m3_s128", ["--afd-micro-batch", "3"], 64, 64, 128, 128, 128),
    # Standard sequences (input=512)
    ("m1_s512", ["--afd-micro-batch", "1"], 64, 64, 128, 512, 128),
    ("m3_s512", ["--afd-micro-batch", "3"], 64, 64, 128, 512, 128),
    # Long sequences (input=2048): attn dominates, M=3 overhead is worse
    ("m1_s2k", ["--afd-micro-batch", "1"], 32, 32, 64, 2048, 128),
    ("m3_s2k", ["--afd-micro-batch", "3"], 32, 32, 64, 2048, 128),
]

all_results = []
for label, afd_args, max_running, concurrency, max_requests, input_len, output_len in configs:
    log.info("=" * 60)
    log.info("Starting: %s (input_len=%d, max_running=%d)", label, input_len, max_running)
    log.info("=" * 60)
    r = run_config(label, afd_args, max_running, concurrency, max_requests, input_len, output_len)
    if r:
        all_results.append(r)

# Summary
log.info("\n" + "=" * 100)
log.info("SEQUENCE LENGTH COMPARISON: M=1 vs M=3")
log.info("=" * 100)
...
with open(os.path.join(LOG_DIR, "seq_len_comparison.json"), "w") as f:
