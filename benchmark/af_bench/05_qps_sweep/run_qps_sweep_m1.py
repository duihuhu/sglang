#!/usr/bin/env python3
"""Run PD+AF M=1 at QPS=2, 500 requests."""
import json, logging, os, socket, subprocess, sys, time

logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] %(message)s")
log = logging.getLogger("qps_sweep_m1")
...
start_server("PF", pf_cmd, pf_env, "qps_sweep_m1_pf.log")
...
start_server("PA", pa_cmd, pa_env, "qps_sweep_m1_pa.log")
...
start_server("DF", df_cmd, df_env, "qps_sweep_m1_df.log")
...
start_server("DA", da_cmd, da_env, "qps_sweep_m1_da.log")
...
router_fh = open(os.path.join(LOG_DIR, "qps_sweep_m1_router.log"), "w")
...
# ── Final table ───────────────────────────────────────────────────────────
print("\n" + "=" * 90)
print("  PD+AF M=1 QPS=2 Benchmark")
...
# Save
with open(os.path.join(LOG_DIR, "qps_sweep_m1_results.json"), "w") as f:
