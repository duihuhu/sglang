#!/usr/bin/env python3
"""Test M=3 vs M=1 WITH CUDA graph enabled to reduce kernel launch overhead.
Key hypothesis: CUDA graph can make attn(batch=21) much faster by eliminating
kernel launch overhead, making M=3 pipeline overlap effective.
"""
import json, logging, os, socket, subprocess, sys, time

logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] %(message)s")
log = logging.getLogger("cuda_graph_test")
...
def run_config(label, extra_args, max_running=64):
    kill_all()
    procs = []
    ...
    # Warmup (important for CUDA graph capture)
    warm_payloads = [
        {"text": f"Hello world {i}, this is a warmup request for CUDA graph capture:", 
         "sampling_params": {"max_new_tokens": 32, "temperature": 0.0}}
        for i in range(16)
    ]
    log.info("%s: Warmup (for CUDA graph capture)...", label)
    def _send(payload):
        try:
            requests.post(f"http://127.0.0.1:{ROUTER_PORT}/generate", json=payload, timeout=180)
        except Exception as e:
            log.warning("Warmup request failed: %s", e)
    with concurrent.futures.ThreadPoolExecutor(max_workers=16) as ex:
        list(ex.map(_send, warm_payloads))
    time.sleep(10)  # Extra time for CUDA graph capture
...
# Test configs
configs = [
    # M=1 without CUDA graph (baseline, same as before)
    ("m1_nocg", ["--afd-micro-batch", "1", "--disable-cuda-graph", "--disable-piecewise-cuda-graph"]),
    # M=1 WITH CUDA graph
    ("m1_cg", ["--afd-micro-batch", "1"]),
    # M=3 WITH CUDA graph (hypothesis: CG reduces kernel launch overhead)
    ("m3_cg", ["--afd-micro-batch", "3"]),
    # M=3 without CUDA graph (for comparison)
    ("m3_nocg", ["--afd-micro-batch", "3", "--disable-cuda-graph", "--disable-piecewise-cuda-graph"]),
]
...
for label, extra_args in configs:
    ...
    log.info("=" * 60)
    r = run_config(label, extra_args)
    if r:
        all_results.append(r)

# Summary
log.info("\n" + "=" * 100)
log.info("CUDA GRAPH COMPARISON")
log.info("=" * 100)
...
with open(os.path.join(LOG_DIR, "cuda_graph_comparison.json"), "w") as f:
