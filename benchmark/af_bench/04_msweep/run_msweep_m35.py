#!/usr/bin/env python3
"""Run only M=3 and M=5 (gap fill). Better cleanup on partial starts."""
...
def launch_servers(m_value):
    ...
    if not wait_port(PA_PORT) or not wait_port(PF_PORT):
        ...
        return None, procs  # Return partial procs for cleanup
...
def main():
    ...
    for m in [3, 5]:
        ...
        procs, partial = launch_servers(m)
        if procs is None:
            print(f"  FAILED to start M={m}, cleaning up partial processes")
            cleanup(partial)
            kill_all()
            time.sleep(5)
            continue
    ...
    # Print summary
    print("\n" + "=" * 70)
    print("  M-Sweep Gap-Fill Summary")
    print("=" * 70)
    for m in [3, 5]:
        r = all_results.get(f"M={m}", {})
        if r:
            print(f"  M={m}: out={r['out_tok_s']} tok/s, TPOT={r['tpot_ms']}ms, TTFT={r['ttft_ms']}ms, OK={r['ok']}")
    ...
    print(f"\nResults merged to msweep_all_results.json")
