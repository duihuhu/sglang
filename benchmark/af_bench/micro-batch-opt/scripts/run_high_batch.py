#!/usr/bin/env python3
"""Test M=1 vs M=2 at high batch sizes (conc=256, 512).

Hypothesis: at large batch (300+), FFN becomes fully compute-bound,
FFN(150) ≈ FFN(300)/2, so M=2 overlap gives real throughput gain.
"""
import asyncio, json, logging, os, socket, subprocess, sys, time, urllib.request

logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] %(message)s")
log = logging.getLogger("highbatch")
...
def start_pdaf(micro_batch: int, async_pipeline: bool = False):
    ...
    def _start(name, perspective, disagg_mode, port, ucx_base, sched_port,
        ...
        fh = open(os.path.join(HERE, f'hb_m{micro_batch}_{name}.log'), 'w')
    ...
    rf = open(os.path.join(HERE, f'hb_m{micro_batch}_router.log'), 'w')
...
async def measure(url, concurrency, n_total):
    prompt = 'Hello ' * 8
    ...
    async with aiohttp.ClientSession(timeout=timeout) as session:
        raw = await asyncio.gather(*[single(session) for _ in range(n_total)])
    wall_s = time.perf_counter() - wall_start
    results = [r for r in raw if r]
    if not results: return None
    tpots = sorted(r['tpot_ms'] for r in results)
    return {
        'n_ok': len(results), 'n_total': n_total,
...
def main():
    ...
    for label, m, async_pipe in configs:
        ...
        for conc in CONCURRENCIES:
            key = f"{label}@{conc}"
            log.info('%s: conc=%d, n=%d, out=%d', key, conc, N_REQUESTS, OUTPUT_TOKENS)
            r = asyncio.run(measure(url, conc, N_REQUESTS))
            if r:
                all_results[key] = r
                log.info('%s: TPOT=%.1fms Thru=%.1f tok/s (%d/%d)',
                         key, r['mean_tpot_ms'], r['throughput_tok_s'], r['n_ok'], r['n_total'])
            else:
    ...
    # Summary
    print('\n' + '=' * 90)
    print(f'  High-Batch Results (out={OUTPUT_TOKENS})')
    print('=' * 90)
    print(f'{"Config":<20} {"TPOT":<12} {"Throughput":<14} {"OK":<10}')
    print('-' * 60)
    for conc in CONCURRENCIES:
        for label, _, _ in configs:
            ...
            if r:
                print(f'{key:<20} {r["mean_tpot_ms"]:<10.1f}ms {r["throughput_tok_s"]:<12.1f}tok/s {r["n_ok"]}/{r["n_total"]}')
            else:
                print(f'{key:<20} FAILED')
    print('=' * 90)

    # Delta at each concurrency
    for conc in CONCURRENCIES:
        m1 = all_results.get(f"M=1@{conc}")
        m2 = all_results.get(f"M=2 async@{conc}")
        if m1 and m2:
            tpot_d = (m2['mean_tpot_ms'] / m1['mean_tpot_ms'] - 1) * 100
            thru_d = (m2['throughput_tok_s'] / m1['throughput_tok_s'] - 1) * 100
            print(f'  conc={conc}: M=2 vs M=1 → TPOT {tpot_d:+.1f}%  Throughput {thru_d:+.1f}%')

    with open(os.path.join(HERE, 'high_batch_results.json'), 'w') as f:
