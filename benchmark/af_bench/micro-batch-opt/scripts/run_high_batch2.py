...
def start_pdaf(micro_batch: int, async_pipeline: bool = False):
    ...
    def _start(name, perspective, disagg_mode, port, ucx_base, sched_port,
        ...
        fh = open(os.path.join(HERE, f'hb2_{tag}_{name}.log'), 'w')
    ...
    rf = open(os.path.join(HERE, f'hb2_m{micro_batch}_router.log'), 'w')
...
def main():
    ...
    for label, m, async_pipe in configs:
        ...
        da_log = os.path.join(HERE, f'hb2_{tag}_da.log')
    ...
    with open(os.path.join(HERE, 'high_batch2_results.json'), 'w') as f:
