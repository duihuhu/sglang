#!/usr/bin/env python3
"""
High-concurrency sweep (256-2048) for all 4 architectures.
Uses short input/output (32/32) to maximize batch size.
Each concurrency level restarts the server to avoid state issues.
"""
import json
import logging
import os
...
log = logging.getLogger("high_conc")
...
INPUT_LEN = 32
OUTPUT_LEN = 32
CONCURRENCY_LEVELS = [256, 512, 768, 1024, 1536, 2048]
...
def run_benchmark_at_concurrency(url, concurrency, label):
    ...
    subprocess.run(cmd, cwd=HERE, capture_output=True, timeout=660)
...
def main():
    ...
    for label, start_fn in configs:
        log.info("\n" + "=" * 80)
        log.info(">>> %s (high conc, in=%d out=%d) <<<", label, INPUT_LEN, OUTPUT_LEN)
        log.info("=" * 80)
        ...
        for conc in CONCURRENCY_LEVELS:
            ...
            if r:
                ...
                log.info("  [%s] conc=%d: TTFT=%.1fms TPOT=%.1fms Thru=%.1f tok/s (%d/%d ok)",
    ...
    # Print summary
    print("\n" + "=" * 120)
    print(f"  High-Concurrency Sweep (256-2048): All Architectures (in={INPUT_LEN}, out={OUTPUT_LEN})")
    print(f"  Model: {MODEL}")
    print("=" * 120)
    header = f"  {'Conc':>5} | {'Config':<8} | {'TTFT(ms)':>9} | {'TPOT(ms)':>9} | {'Thru(tok/s)':>12} | {'P95_TTFT':>9} | {'P95_TPOT':>9} | {'OK':>8}"
    print(header)
    print("  " + "-" * 105)
    ...
    for conc in CONCURRENCY_LEVELS:
        for label, _ in configs:
            ...
            if r and not r.get("error"):
                ...
                print(f"  {conc:>5} | {label:<8} | {'FAILED':^60}")
        if conc < CONCURRENCY_LEVELS[-1]:
            print("  " + "-" * 105)

    print("=" * 120)

    output_file = os.path.join(LOG_DIR, "high_conc_sweep_results.json")
