...
# This file may live under <repo>/energy-profile-bundle/bash-test/ or
# <repo>/benchmark/test_motivation/lt/bash-test/ — resolve checkout root by finding python/sglang.
_SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))


def _find_sglang_repo_root(start_dir: str) -> str:
    d = os.path.abspath(start_dir)
    for _ in range(16):
        if os.path.isfile(os.path.join(d, "python", "sglang", "__init__.py")):
            return d
...
_REPO_ROOT = _find_sglang_repo_root(_SCRIPT_DIR)
_PATCHED_SGLANG_PYTHON_DEFAULT = os.path.abspath(
    os.path.join(_SCRIPT_DIR, "..", "patched-sglang", "python")
)


def _pythonpath_for_sglang(*, use_patched: bool, patched_override: str) -> str:
    """Put patched `.../python` first so `import sglang` resolves to overlay modules."""
    parts: List[str] = []
    patched = (patched_override or "").strip()
    if use_patched and not patched:
        patched = (os.environ.get("SGLANG_PATCHED_PYTHON") or "").strip()
    if use_patched and not patched:
        patched = _PATCHED_SGLANG_PYTHON_DEFAULT
    if use_patched and patched:
        patched_abs = os.path.abspath(patched)
        if os.path.isdir(os.path.join(patched_abs, "sglang")):
            parts.append(patched_abs)
    repo_py = os.path.join(_REPO_ROOT, "python")
    parts.append(repo_py)
...
_PIPELINE_PY = os.path.join(_SCRIPT_DIR, "pd_latency_pipeline.py")

from pd_latency_pipeline import (
    load_done_keys_from_wide_csv,
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=int,
        default=512,
        help="d-bucket-count for pd_latency_pipeline.py nvtx-rep (usually equals max output_len).",
    )
    parser.add_argument(
        "--d-pick-positions",
        type=str,
        default="64,256,512",
        help="d-pick-positions for pd_latency_pipeline.py nvtx-rep. Must be subset of [1..nsys-d-bucket-count].",
    )
    parser.add_argument(
        "--nsys-python",
        type=str,
        default=sys.executable,
        help="Python executable used to run pd_latency_pipeline.py nvtx-rep.",
    )
    parser.add_argument(
        "--max-background-jobs",
        type=int,
        default=1,
        help=(
            "Max concurrent background nvtx-rep jobs (each runs `nsys stats`). "
    ...
    parser.add_argument(
        "--patched-sglang-python",
        type=str,
        default="",
        help=(
            "Directory containing the `sglang` package tree (e.g. lt/patched-sglang/python). "
            "Prepended to PYTHONPATH for the server and bench child processes. "
            "Default: env SGLANG_PATCHED_PYTHON, else lt/patched-sglang/python if that exists."
        ),
    )
    parser.add_argument(
        "--no-patched-sglang",
        action="store_true",
        help="Only use checkout python/ on PYTHONPATH (do not prepend patched-sglang).",
    )
    ...
    if args.only_merge:
        out_csv_path = merge_existing_results(
            work_dir=work_dir,
        ...
        pivot_out_dir = run_pivot_wide(out_csv_path)
    ...
    if skip_done_wide_csv:
        ...
        done_keys = load_done_keys_from_wide_csv(skip_csv_path)
        if done_keys:
            before = len(jobs)
            jobs = [
                j
    ...
    py_path = _pythonpath_for_sglang(
        use_patched=not bool(getattr(args, "no_patched_sglang", False)),
        patched_override=str(getattr(args, "patched_sglang_python", "") or ""),
    )
    print(f"[info] SGLang repo root: {_REPO_ROOT}")
    print(f"[info] PYTHONPATH for server/bench (order matters): {py_path}")
