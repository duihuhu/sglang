#!/usr/bin/env python3
"""
Decode Pipeline Coupled Profiling — MoE Heterogeneous TP (DA=TP1, DF=TP4).

Profiles decode iteration latency + energy for Qwen3-30B-A3B with:
  - DA (Attention): TP=1 (1 GPU)
  - DF (FFN):       TP=4 (4 GPUs)

Architecture (8 GPUs):
    PF(ffn, TP=2, GPU0,1) + PA(attn, TP=1, GPU2)  (prefill, max freq)
    DF(ffn, TP=4, GPU3,4,5,6) + DA(attn, TP=1, GPU7)  (decode, freq swept)

Output format (TSV):
    tp_a  tp_f  M  f_A  f_F  input_len  batch_size  iter_lat_us  DA_energy_mj  DF_energy_mj

Usage:
    /workspace/env/sglang-tier/bin/python bench_decode_pipeline_moe_1a4f.py --quick
    /workspace/env/sglang-tier/bin/python bench_decode_pipeline_moe_1a4f.py
"""
...
log = logging.getLogger("bench_moe_1a4f")
...
# GPU layout matching run_moe_bench.py pdaf_asym:
#   Prefill: PF(ffn, TP=2, GPU0,1) + PA(attn, TP=1, GPU2)
#   Decode:  DF(ffn, TP=4, GPU3,4,5,6) + DA(attn, TP=1, GPU7)
P_CVD = "0,1,2"
D_CVD = "3,4,5,6,7"
DA_NVML_GPUS = [7]
DF_NVML_GPUS = [3, 4, 5, 6]
P_FFN_TP = 2
P_ATTN_TP = 1
...
def start_pdaf_stack(M: int, log_dir: Path):
    """Start MoE heterogeneous TP PDAF stack + router.
    ...
    def _cmd(port, perspective, disagg, tp, base_gpu_id,
        ...
        cmd = [PYTHON, "-m", "sglang.launch_server",
               ...
               "--mem-fraction-static", mem_frac] + common
    ...
    # Prefill: PF(ffn, TP=2, GPU0,1) + PA(attn, TP=1, GPU2)
    _start("pf", _cmd(PF_PORT, "ffn", "prefill", tp=P_FFN_TP, base_gpu_id=0,
                       attn_tp=P_ATTN_TP, ffn_tp=P_FFN_TP, mem_frac="0.70"),
           _env(P_CVD, UCX_P, SCHED_P, peer_device=2, nvml_indices="0,1"))
    time.sleep(3)
    _start("pa", _cmd(PA_PORT, "attn", "prefill", tp=P_ATTN_TP, base_gpu_id=2,
                       attn_tp=P_ATTN_TP, ffn_tp=P_FFN_TP, mem_frac="0.70"),
           _env(P_CVD, UCX_P, SCHED_P, peer_device=0, nvml_indices="2",
                ffn_host="127.0.0.1"))
    time.sleep(3)

    # Decode: DF(ffn, TP=4, GPU3-6, base=0) + DA(attn, TP=1, GPU7, base=4)
    _start("df", _cmd(DF_PORT, "ffn", "decode", tp=TP_F, base_gpu_id=0,
                       attn_tp=TP_A, ffn_tp=TP_F),
           _env(D_CVD, UCX_D, SCHED_D, peer_device=4, nvml_indices="3,4,5,6"))
    time.sleep(5)
    _start("da", _cmd(DA_PORT, "attn", "decode", tp=TP_A, base_gpu_id=4,
                       attn_tp=TP_A, ffn_tp=TP_F),
           _env(D_CVD, UCX_D, SCHED_D, peer_device=0, nvml_indices="7",
                ffn_host="127.0.0.1"))
    ...
    log.info("PDAF MoE hetero-TP stack ready (M=%d, DA-TP%d, DF-TP%d): router at %d",
...
def should_skip_config(il, bs, f_a, f_f):
    # MoE DA-TP1+DF-TP4: DF communication overhead is high, skip aggressively
    if il >= 4096 and bs >= 32:
        return True
    if il >= 4096 and bs >= 16 and (f_a < 690 or f_f < 690):
        return True
    if il >= 2048 and bs >= 128:
        return True
    if il >= 2048 and bs >= 96 and (f_a < 930 or f_f < 930):
        return True
    if il >= 1024 and bs >= 128 and (f_a < 690 or f_f < 690):
        return True
    if bs >= 256:
        return True
    return False
...
def main():
    ...
    log.info("MoE Decode Pipeline Profiling — DA-TP%d + DF-TP%d", TP_A, TP_F)
