#!/usr/bin/env python3
"""
Decode Pipeline Coupled Profiling — Heterogeneous TP version (DA=TP2, DF=TP4).

Profiles decode with asymmetric tensor parallelism:
  - DA (Attention): TP=2 (2 GPUs)
  - DF (FFN):       TP=4 (4 GPUs)

Architecture (8 GPUs):
    PF(ffn, TP=1) on GPU 0  +  PA(attn, TP=1) on GPU 1  (prefill, max freq)
    DF(ffn, TP=4) on GPU 2,3,4,5  +  DA(attn, TP=2) on GPU 6,7  (decode, freq swept)

Output format (TSV, same as v1/v2):
    tp_a  tp_f  M  f_A  f_F  input_len  batch_size  iter_lat_us  DA_energy_mj  DF_energy_mj

Note: tp_a=2 (DA), tp_f=4 (DF) for all rows in this dataset.
      DA_energy_mj = sum of DA GPUs (2 GPUs) per iteration
      DF_energy_mj = sum of DF GPUs (4 GPUs) per iteration

Usage:
    /workspace/env/sglang-tier/bin/python bench_decode_pipeline_hetero.py --quick
    /workspace/env/sglang-tier/bin/python bench_decode_pipeline_hetero.py
"""
...
log = logging.getLogger("bench_pipeline_hetero")
...
# Heterogeneous TP: DA=TP2, DF=TP4
TP_A = 2  # Attention TP
TP_F = 4  # FFN TP

# GPU layout:
#   Prefill: PF(ffn, TP=1, GPU0) + PA(attn, TP=1, GPU1) — minimal for feeding decode
#   Decode:  DF(ffn, TP=4, GPU2,3,4,5) + DA(attn, TP=2, GPU6,7)
P_CVD = "0,1"          # prefill: PF(0) + PA(1)
D_CVD = "2,3,4,5,6,7"  # decode:  DF(2,3,4,5) + DA(6,7)
DA_NVML_GPUS = [6, 7]
DF_NVML_GPUS = [2, 3, 4, 5]
P_TP = 1  # prefill TP (minimal)
...
def start_pdaf_stack(M: int, log_dir: Path):
    """Start heterogeneous TP PDAF stack + router.
    
    Layout:
      Prefill: PF(ffn, TP=1, GPU0) + PA(attn, TP=1, GPU1)
      Decode:  DF(ffn, TP=4, GPU2,3,4,5) + DA(attn, TP=2, GPU6,7)
    """
    ...
    def _cmd(port, perspective, disagg, tp, base_gpu_id, attn_tp=None, ffn_tp=None):
    ...
    # Prefill: PF(ffn, TP=1, GPU0) + PA(attn, TP=1, GPU1) — minimal prefill
    _start("pf", _cmd(PF_PORT, "ffn", "prefill", P_TP, 0),
           _env(P_CVD, UCX_P, SCHED_P, peer_device=1, nvml_indices="0"))
    time.sleep(2)
    _start("pa", _cmd(PA_PORT, "attn", "prefill", P_TP, 1),
           _env(P_CVD, UCX_P, SCHED_P, peer_device=0, nvml_indices="1",
                ffn_host="127.0.0.1"))
    time.sleep(2)

    # Decode: DF(ffn, TP=4, base=0, GPU2-5) + DA(attn, TP=2, base=4, GPU6-7)
    _start("df", _cmd(DF_PORT, "ffn", "decode", TP_F, 0, attn_tp=TP_A),
           _env(D_CVD, UCX_D, SCHED_D, peer_device=4, nvml_indices="2,3,4,5"))
    time.sleep(5)
    _start("da", _cmd(DA_PORT, "attn", "decode", TP_A, 4, ffn_tp=TP_F),
           _env(D_CVD, UCX_D, SCHED_D, peer_device=0, nvml_indices="6,7",
                ffn_host="127.0.0.1"))

    log.info("Waiting for all 4 processes to become healthy (DA-TP%d, DF-TP%d)...",
             TP_A, TP_F)
    ...
    log.info("PDAF hetero-TP stack ready (M=%d, DA-TP%d, DF-TP%d): router at %d",
             M, TP_A, TP_F, ROUTER_PORT)
    return procs
...
def should_skip_config(il, bs, f_a, f_f):
    """Skip configs that will certainly timeout or OOM for DA-TP2+DF-TP4.
    
    DF-TP4 has high communication overhead at low frequencies with large batches.
    PF-TP1 has limited KV cache capacity (~64K tokens on 1 GPU).
    """
    # DF-TP4 at low freq + large BS: communication dominates, certain timeout
    if il >= 4096 and bs >= 64:
        return True
    if il >= 4096 and bs >= 32 and (f_a < 690 or f_f < 690):
        return True
    if il >= 2048 and bs >= 256:
        return True
    if il >= 2048 and bs >= 128 and (f_a < 930 or f_f < 930):
        return True
    if il >= 1024 and bs >= 256 and (f_a < 930 or f_f < 930):
        return True
    # Extremely large batch at low freq is futile
    if bs >= 256 and (f_a < 450 or f_f < 450):
        return True
    return False
...
def run_sweep(args):
    ...
    if out_path.exists() and os.path.getsize(out_path) > 0:
        with open(out_path) as f:
            for line in f:
                if line.startswith("tp_a\t"):
                    continue
                parts = line.strip().split("\t")
                if len(parts) >= 7:
                    completed.add(tuple(parts[:7]))
    ...
    if write_header:
        f_out.write("tp_a\ttp_f\tM\tf_A\tf_F\tinput_len\tbatch_size\t"
                ...
                for f_a in args.freqs:
                    for f_f in args.freqs:
                        if should_skip_config(il, bs, f_a, f_f):
                            continue
                        key = (str(TP_A), str(TP_F), str(M), str(f_a), str(f_f), str(il), str(bs))
    ...
    for M in sorted(by_m.keys()):
        ...
        log.info("=" * 60)
        log.info("Phase M=%d: %d configs (DA-TP%d, DF-TP%d)", M, len(m_configs),
                 TP_A, TP_F)
        log.info("=" * 60)
        ...
        for cfg_idx, (f_a, f_f, il, bs) in enumerate(m_configs):
            ...
            try:
                ...
                f_out.write(f"{TP_A}\t{TP_F}\t{M}\t{f_a}\t{f_f}\t{il}\t{bs}\t"
...
def main():
    parser = argparse.ArgumentParser(
        description="Decode pipeline coupled profiling (DA-TP2 + DF-TP4)")
    parser.add_argument("--output", default=str(
        SCRIPT_DIR / "paper" / "decode_pipeline_hetero_2a4f.txt"))
    parser.add_argument("--log-dir", default=str(
        SCRIPT_DIR / "logs" / "decode_pipeline_hetero"))
    ...
    log.info("Decode Pipeline Coupled Profiling — DA-TP%d + DF-TP%d", TP_A, TP_F)
