...
def profiling_worker(server_args, port_args, bench_args, gpu_id, tp_rank):
    ...
    # Use physical GPU ID for NVML: CUDA_VISIBLE_DEVICES at this point is a single GPU index
    # set by maybe_reindex_device_id (e.g. '0', '1', '2', '3')
    import os
    _cvd = os.environ.get('CUDA_VISIBLE_DEVICES', '')
    # _cvd might be single digit like '2' or multi like '0,1,2,3' (if spawn didn't inherit)
    if _cvd and ',' not in _cvd:
        _phys_gpu = int(_cvd)
    else:
        _phys_gpu = tp_rank  # fallback: use tp_rank as physical GPU index
    ctrl = DVFSController(_phys_gpu)
    rank_print(f"  [DVFS] rank={tp_rank}, CVD={_cvd}, phys_gpu={_phys_gpu}")
    device = model_runner.device
