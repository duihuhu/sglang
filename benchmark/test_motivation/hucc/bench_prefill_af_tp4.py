#!/usr/bin/env python3
"""
Prefill Attention/FFN latency + energy profiling using real SGLang model.
...
profiles layer.self_attn and layer.mlp with the NVML energy counter.
...
DEFAULT_INPUT_LENS = [128, 256, 512, 1024, 2048, 4096, 8192, 16384, 32000]
...
def profiling_worker(server_args, port_args, bench_args, gpu_id, tp_rank):
    ...
    if tp_rank == 0:
        ...
        if write_header:
            f_out.write("tp\tinput_len\tgpu_clock\tbatch_size\t"
                        "P_A_lat\tP_F_lat\tP_A_energy\tP_F_energy\n")
            f_out.flush()
...
def main():
    parser = argparse.ArgumentParser(description="Prefill A/F latency + energy profiling")
