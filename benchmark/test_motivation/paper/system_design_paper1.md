...
|------|---------|---------|--------|-------------|
| tp=1, in=128, bs=1, 1410MHz | 313 | 583 | 1.86 | F 是瓶颈 → 多给 PF GPU |
| tp=1, in=8192, bs=1, 1410MHz | 12,040 | 25,450 | 2.11 | F 是瓶颈 → 多给 PF GPU |
| tp=1, in=16384, bs=1, 1410MHz | 33,770 | 51,044 | 1.51 | F 是瓶颈但差距缩小 |
| tp=4, in=128, bs=1, 1410MHz | 307 | 234 | **0.76** | A 是瓶颈 → 多给 PA GPU |
| tp=8, in=128, bs=4, 1410MHz | 316 | 1,748 | **5.52** | F 强瓶颈 → 大幅倾斜给 PF |
| tp=8, in=8192, bs=4, 1410MHz | 6,933 | 94,370 | **13.61** | F 极强瓶颈 (memory-bound) |

**当负载以长请求为主时，ILP 倾向给 PF 分配更多 GPU (降低 f̄_PF 的同时保持吞吐); 短请求为主时反之。** tp=8 时 F/A 比极大（5~14x），说明 F 完全 memory-bound，权重被切分到 8 块 GPU 后计算量极小但访存量不变，ILP 会大幅倾斜给 PF。
...
═══════════════════════════════════════════════════════════════════
 ...
 [T1-2] 运行 Prefill Profiling                        ✅ 已完成
   产出: prefill_data.txt (1038 行, 含 P_A_energy/P_F_energy)
 ...
 [T1-4] 运行 Decode Profiling                         ✅ 已完成
   产出: decode_data.txt (2552 行, 含 D_A_energy/D_F_energy)
...
═══════════════════════════════════════════════════════════════════
 ...
 [T2-3] Motivation 论证撰写                           ⏱ 1 天
   输入: [T1-7] Pareto 图 + decode_data.txt + prefill_data.txt
   已完成分析 (见 Section 二·五 ~ 二·九):
     ✅ BiScale U型陷阱量化 (Decode SLO×1.5 时 BiScale 比 OptB 多耗能 14~25%)
     ✅ AF vs 原始BiScale 系统级收益 (Decode 5~9%, Prefill tp=8 14.7%)
     ✅ AF vs OptB 纯粹收益 (Decode 5~9% @SLO×1.0, 饱和到 2~4%)
   ...
   已完成图表:
     ...
     ✅ AFlex vs OptB 放大图 + P25~P75 band → fig2b_aflex_vs_optb_zoom.png (plot_fig2b_optb_only.py)
...
```
可并行启动:
  [T2-1] 能耗模型拟合 (数据已就绪: decode_data.txt + prefill_data.txt)
