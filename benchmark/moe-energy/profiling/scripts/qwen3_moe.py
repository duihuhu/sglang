...
class Qwen3MoeSparseMoeBlock(nn.Module):
    def __init__(
        ...
        # Profiling-only static Router Top-K override. This deliberately changes
        # the model function and must never be enabled implicitly in production.
        router_top_k = int(os.environ.get("SGLANG_PROFILE_ROUTER_TOP_K", config.num_experts_per_tok))
        if not 1 <= router_top_k <= config.num_experts_per_tok:
            raise ValueError(
                f"SGLANG_PROFILE_ROUTER_TOP_K must be in [1, {config.num_experts_per_tok}], "
                f"got {router_top_k}"
            )
        self.topk = TopK(
            top_k=router_top_k,
