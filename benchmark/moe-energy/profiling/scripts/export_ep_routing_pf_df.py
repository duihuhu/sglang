#!/usr/bin/env python3
"""Export forced-routing F-EP JSONL into PF/DF-style TSV matrices."""
...
class Summary:
    ...
    def render(self) -> str:
        return (
            f"export-ep-routing files_scanned={self.files_scanned} "
...
def load_ep_rows(raw_root: Path, summary: Summary) -> dict[tuple, dict[str, Any]]:
        ...
        with path.open() as handle:
            for line_number, line in enumerate(handle, 1):
                ...
                if row.get("schema_version") != SCHEMA_VERSION or row.get("status") != "ok":
                    summary.skipped += 1
                    continue
                if row.get("component") != "F" or row.get("parallel_mode") != "moe_ep":
                    summary.skipped += 1
                    continue
...
def export_matrices(raw_root: Path, output_dir: Path) -> Summary:
    summary = Summary()
    rows = load_ep_rows(raw_root, summary)
    ...
    for phase, routing_mode in sorted(grouped):
        ...
        for row in sorted(subset, key=lambda r: (int(r["world_size"]), int(r["length"]), int(r["batch"]), int(r["freq_mhz"]))):
            lines.append(
                "\t".join(
                    [
                        "ep",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--raw-root",
        type=Path,
        default=profile_root / "data" / "EP" / "raw_v2",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=profile_root / "data" / "EP",
    )
