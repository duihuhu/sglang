#!/usr/bin/env python3
"""Single-MoE-call dynamic Top-K using invalid masked trailing slots."""
...
def onepass(mlp,x,logits,w8,ids8,mask,low_k):
 from sglang.srt.layers.moe.topk import StandardTopKOutput
 from sglang.srt.distributed.communication_op import moe_expert_parallel_all_reduce
 per_k=torch.where(mask,torch.full_like(mask,8,dtype=torch.int64),torch.full_like(mask,low_k,dtype=torch.int64));slots=torch.arange(8,device=x.device).unsqueeze(0);valid=slots<per_k.unsqueeze(1);ids=torch.where(valid,ids8,torch.full_like(ids8,-1));w=torch.where(valid,w8,torch.zeros_like(w8));w=w/w.sum(-1,keepdim=True).clamp_min(1e-12);o=mlp.experts(x,StandardTopKOutput(w,ids,logits))
 ...
 try:
  ...
  for phase,M in CASES:
   ...
   with torch.no_grad(),forward_context(ForwardContext(attn_backend=runner.attn_backend)):
    logits,w8,ids8=full_top8(mlp,x);mask_cache={r:shared_mask(importance,r) for r in RATIOS};ref=onepass(mlp,x,logits,w8,ids8,torch.ones(M,device=x.device,dtype=torch.bool),8);torch.cuda.synchronize()
    # Correlation diagnostic against static Top-4 token error.
    out4=onepass(mlp,x,logits,w8,ids8,torch.zeros(M,device=x.device,dtype=torch.bool),4);err=(out4.float()-ref.float()).norm(dim=-1)/ref.float().norm(dim=-1).clamp_min(1e-12);corr=torch.corrcoef(torch.stack([importance,err]))[0,1].item()
    for ratio in RATIOS:
     mask=mask_cache[ratio]
     for low_k in LOW_KS:
      out=onepass(mlp,x,logits,w8,ids8,mask,low_k);quality=qm(out,ref,mask);torch.cuda.synchronize()
      def target():
       target_logits,target_w8,target_ids8=full_top8(mlp,x)
       return onepass(mlp,x,target_logits,target_w8,target_ids8,mask,low_k)
      lat,rl,re,ene,actual=profile_world(target,ctrl,5,20,8,phase,3)
      if rank==0:
       row={'status':'ok','implementation':'onepass_masked_width8','phase':phase,'M':M,'importance_proxy':'pre_norm_attention_output_l2','important_ratio':ratio,'important_top_k':8,'normal_top_k':low_k,'effective_avg_top_k':ratio*8+(1-ratio)*low_k,'physical_topk_width':8,'latency_us':lat,'energy_total_mj':ene,'energy_per_token_mj':ene/M,'latency_per_rank_us':rl,'energy_per_rank_mj':re,'quality':quality,'importance_error_pearson':corr,'actual_repeat':actual};f.write(json.dumps(row,separators=(',',':'))+'\n');f.flush()
