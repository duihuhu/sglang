#!/usr/bin/env python3
"""Controlled EP4 boundary scan with exact assignments on each cold rank."""
...
COLD=[64,16,8,4,2,1,0]
def worker(sa,pa,a,gpu,rank):
 from sglang.srt.layers.moe import initialize_moe_config
 from sglang.srt.layers.dp_attention import set_is_extend_in_batch
 from sglang.srt.model_executor.forward_context import ForwardContext,forward_context
 initialize_moe_config(sa);runner=load_runner(sa,pa,gpu,rank);set_is_extend_in_batch(a.phase=='prefill');ctrl=NvmlController(gpu);layer=runner.model.model.layers[0];order=list(COLD);np.random.default_rng(a.order_seed).shuffle(order);f=Path(a.output).open('a',buffering=1) if rank==0 else None
 try:
  ...
  for M in a.tokens:
   ...
   for cold in order:
    layer.mlp._forced_cold_assignments_per_rank=cold;old=forced_routing(layer.mlp,'hot_rank_exact_cold',4)
    with torch.no_grad(),forward_context(ForwardContext(attn_backend=runner.attn_backend)):
     hs,_=layer.post_attention_layernorm(hidden,residual);routing=routing_summary(layer.mlp,hs,4)
     def target():x,_=layer.post_attention_layernorm(hidden,residual);return layer.mlp(x,fb)
     lat,rl,re,ene,actual=profile_world(target,ctrl,10,50,4,a.phase,3)
    layer.mlp.topk.forward=old
    if rank==0:
     total=M*8;row={'status':'ok','phase':a.phase,'M':M,'cold_assignments_per_rank':cold,'hot_assignments':total-3*cold,'total_assignments':total,'hot_fraction':(total-3*cold)/total,'latency_us':lat,'latency_per_rank_us':rl,'energy_total_mj':ene,'energy_per_rank_mj':re,'actual_repeat':actual,'replicate':a.replicate,'block':a.block,'order_seed':a.order_seed,'routing':routing};f.write(json.dumps(row,separators=(',',':'))+'\n');f.flush()
...
def main():
 ...
 p=argparse.ArgumentParser();ServerArgs.add_cli_args(p);p.set_defaults(model_path=DEFAULT_MODEL,tp_size=4,ep_size=4,moe_runner_backend='triton',moe_a2a_backend='none',cuda_graph_backend_decode='disabled',cuda_graph_backend_prefill='disabled',disable_custom_all_reduce=True,max_total_tokens=600000,max_running_requests=4096);p.add_argument('--phase',required=True);p.add_argument('--tokens',type=int,nargs='+',default=[512,1024,4096]);p.add_argument('--output',required=True);p.add_argument('--replicate',type=int,required=True);p.add_argument('--block',required=True);p.add_argument('--order-seed',type=int,required=True);a=p.parse_args();sa=ServerArgs.from_cli_args(a);_set_envs_and_config(sa);pa=PortArgs.init_new(sa);ps=[]
 for r in range(4):
